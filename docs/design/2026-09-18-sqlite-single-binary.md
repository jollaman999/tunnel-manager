...
## 결정 6 - HTTPS 를 기본으로 하고 인증서를 데이터베이스에 둔다

웹 화면에서 SSH 비밀번호를 입력받고 세션 쿠키를 주고받는데 그것이 평문으로 오갔다.

### 인증서는 데이터베이스에, 개인키는 암호문으로

인증서와 개인키가 `settings` 옆 테이블에 들어간다. 데이터베이스 파일 하나가 이 설치의
전부라는 성질을 인증서도 따른다.

**개인키는 `crypto.Cipher` 로 암호화해서 저장한다.** SSH 비밀번호와 같은 키다. 평문으로 두면
데이터베이스 파일 하나로 개인키까지 넘어간다. 키 파일이 따로 있는 값어치가 여기서도 같다.

첫 기동에 자체 서명 인증서를 만들어 넣는다. 유효기간은 5년이다.

처음에는 825일로 두고 "브라우저가 그보다 긴 것을 거부한다" 고 적었는데 **틀렸다.** Apple 이
거는 한도는 398일이고(2020-09-01 이후 발급분), 같은 문서가 **"사용자나 관리자가 직접 추가한
Root CA 에서 나온 인증서에는 적용되지 않는다"** 고 못 박는다. 우리 인증서가 바로 그것이다.
직접 신뢰시키지 않는 운영자는 경고를 클릭해 넘기는 것이라 검증 자체를 건너뛴다.

그래서 기간은 규칙이 아니라 운영자를 보고 정한다. 5년이면 대부분의 설치 수명을 덮고, 10년처럼
"영원히" 가 되지는 않는다. 그 사이 호스트명이나 주소가 바뀌면 SAN 이 안 맞아 어차피 새로
만들어야 한다.

### 한 포트에서 평문을 받아 돌려보낸다

포트를 둘 쓰지 않는다. `api.port` 하나가 TLS 를 받고, 평문 HTTP 로 온 요청은 같은 주소의
`https://` 로 돌려보낸다.

가르는 방법은 첫 바이트다. TLS 핸드셰이크는 `0x16` 으로 시작하고 평문 HTTP 는 `GET` 같은
ASCII 메서드로 시작한다. 소비하지 않고 엿보면 갈린다.

**어려운 것은 가르는 것이 아니라 가르는 동안이다.**

| 함정 | 무엇이 되나 |
|------|-------------|
| Accept 루프 안에서 직접 엿보기 | 연결 하나가 침묵하면 서버 전체가 멈춘다 |
| 엿보기에 읽기 데드라인이 없음 | 같은 결과 |
| 엿본 뒤 데드라인을 안 푼다 | 정상 연결이 나중에 뜬금없이 끊긴다 |
| 엿본 바이트를 잃는다 | 요청이 깨진다 |
| 닫힌 채널에 보낸다 | 패닉 |

세 번째는 이 저장소가 `internal/tunnel/ssh.go` 에서 이미 한 번 겪고 고친 것이다
(`defer conn.SetDeadline(time.Time{})`).

그래서 분류는 연결마다 goroutine 에서 하고, 데드라인을 걸고, 엿본 뒤 푼다. 분류된 연결은
채널로 넘겨 두 개의 리스너처럼 보이게 만든다.

### HTTPS 를 끌 수 있어야 한다

자체 서명 인증서가 문제를 일으키는 자리가 있다. 그때 손이 닿지 않으면 화면에도 못 들어간다.
설정으로 끄면 평문으로 돈다. 기본은 켜짐이다.

## 결정 7 - 로그는 파일에 두고 화면으로 읽는다

"서버에 들어가지 않고 로그를 본다" 가 목적이라면 저장을 옮길 필요가 없다. 파일에 그대로 쓰고
화면이 그 파일을 읽는다.

데이터베이스에 넣지 않는 이유는 셋이다.

1. **로거가 데이터베이스보다 먼저 선다.** 로거를 두 번 세우는 이유가 "데이터베이스를 못 열었다"
   를 말할 곳이 필요해서인데 (결정 4), 로그가 거기 있으면 그 말을 할 데가 없다
2. **커넥션이 하나다** (결정 2). 로그 한 줄마다 실제 작업과 같은 줄에 선다
3. **재귀가 생긴다.** gorm 이 쿼리를 로그에 남기는데 로그를 남기는 것이 쿼리다

파일은 끝에서부터 읽는다. 50MB 로그에서 마지막 200줄을 요청했을 때 프로세스 상주 메모리가
5.2MB 에서 5.3MB 로만 움직였다. 통째로 읽었다면 50MB 가 올라왔을 것이다.

파싱하지 못한 줄도 써진 그대로 돌려준다. 이상해 보이는 줄이야말로 읽을 가치가 있는 줄이다.
