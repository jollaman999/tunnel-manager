...
サーバーが待ち受けるよう保存されているポート (`api_port`) と同じものも、起動が別のポートへ
移ったときに今待ち受けているポートと同じものも同様です。逆向きも
同じで、設定画面の保存や設定のインポートで `api_port` を変えるとき、ローカルフォワードが開く
ポートなら `409` で拒否し、何も保存しません。拒否にはそのローカルフォワードと `suggested_port`
(そのポートより上で、ローカルフォワード、保存されている `api_port`、今待ち受けているポート、
求められた `api_port` の
どれも使っていない最初のポート) が載り、設定画面はローカルフォワードを、保存のときは代わりに
...
| 設定 | 規則 |
|------|------|
| `api_port` | 1 から 65535。ローカルフォワードが開くポートに変えると `409` で拒否されます。[ローカルフォワード](#ローカルフォワード)を参照してください |
...
画面にまたアクセスできます。既定の API ポートを開くローカルフォワードがあると次の起動が別の
ポートで待ち受けることがあるため、そのフォワードを警告（`settings.default_port_held_by_forward`）で
知らせます。
