...
| サービスポート | `service_ip`, `service_port`, `local_port`, `bind_address` | 公開したいサービスと、それを担当するすべての Host の上に開くポートです。`bind_address` はそのポートを Host のどのアドレスで開くよう要求するかで、与えなければ `0.0.0.0` です。 |
...
要求するアドレスはサービスポートの `bind_address` で、与えなければ `0.0.0.0` です。
[サービスポート](#サービスポート)を参照してください。Host 側の待ち受けが本当にそのアドレスに
...
| メソッド | パス | 何をするか |
|----------|------|------------|
| `POST` | `/api/service-port` | サービスポートを作ります。`bind_address` は省略でき、省くと `0.0.0.0` です。`assign_to_all_hosts` も省略でき、省くと保存されているすべての Host に与えられ、false を送ると誰も担当しない状態で登録されます |
...
| `PUT` | `/api/service-port/:id` | サービスポートを更新します。`service_ip`, `service_port`, `local_port` はすべて必須で、`bind_address` を省くと `0.0.0.0` です |
| `DELETE` | `/api/service-port/:id` | サービスポートと、それを指す割り当てを削除します |

**`bind_address` は、転送されたポートを Host のどのアドレスで開くよう要求するかです。**
...
できる前にすべてのサービスポートが要求していた値なので、以前のバージョンで保存された
サービスポートの届く範囲は変わりません。
...
[転送ポートに到達できるかどうか](#転送ポートに到達できるかどうか)を参照してください。
ループバックにバインドしたサービスポートはそこで `unreachable` と報告されますが、これは
測定が正しく働いた結果です - その測定はこのプロセスから Host へ接続してみるものであり、
ループバックに開いたポートはそれに応答しません。

アドレスを変えると、そのサービスポートを担当するトンネルは作り直されます。アドレスは
トンネルを組み立てる値の一部だからです。
...
`<バインドアドレス>:<ローカルポート>` で、サービスポートが別のアドレスを指定していなければ
