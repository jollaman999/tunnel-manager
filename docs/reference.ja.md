...
`service_address:service_port` に接続して、バイトを両方向にコピーします。Tunnel Manager しか
接続できないサービスを Host から使えるようにするのが、この仕組みです。
[ローカルフォワード](#ローカルフォワード)と [Host の SOCKS5 プロキシ](#host-の-socks5-プロキシ)
だけは向きが逆で、ポートはこのマシンに開き、接続は Host から出ていきます。

| 登録するもの | 項目 | 何か |
|--------------|------|------|
| Host | `address`, `port`, `user`, `private_key`, `key_passphrase`, `password`, `description`, `enabled`, `socks_enabled`, `socks_port`, `socks_bind_scope`, `socks_allowed_sources` | Tunnel Manager がログインする SSH サーバーです。秘密鍵でも、パスワードでも、その両方でもログインできますが、どちらか一方は必ず要ります。鍵も、そのパスフレーズも、パスワードも、すべて暗号化して保存します。`socks_` の項目は、この Host が持てる [SOCKS5 プロキシ](#host-の-socks5-プロキシ)です。 |
| サービスポート | `service_address`, `service_port`, `local_port` | 公開したいサービスと、それを担当するすべての Host の上に開くポートです。 |
...
| ローカルフォワード | `local_port`, `bind_scope`, `target_address`, `target_port`, `description` | このマシンに開くポートです。そこへの接続を 1 台の Host の SSH 接続を通して、Host から見た `target_address:target_port` へ運びます。その Host だけに属します。 |

`address`、`service_address`、そしてローカルフォワードの `target_address` は、ホスト名か IPv4、
IPv6 のアドレスを受け取ります。名前は保存したときではなく接続するたびに引くので、名前が別の
アドレスに移れば次の接続から追いかけます。`address` と `service_address` はこのマシンが引き、
`target_address` は Host が引きます。IPv6 アドレスは `2001:db8::1` のようにそのまま書き、接続の
ときに要る角かっこは、アドレスを使う側で付けます。`fe80::1%eth0` のようなゾーン付きは受け付けま
せん。

**この 3 つの項目は、v3.14.0 より前は `ip`、`service_ip`、`target_ip` でした。** まだ古い名前を
送るリクエストは、その項目を黙って捨てるのではなく、`400` と `request.field_renamed` で拒否し、
古い項目と新しい項目の名前を示します。以前のリリースがエクスポートしたファイルはそのまま
インポートできます。インポートは古い名前も読みます。

**Host が有効で、それ自体もオンになっている割り当て 1 つが、トンネル 1 本です。** サービスポートを 3 つ担当する
...
| 状態 | 何か | どこから来るか |
|------|------|----------------|
| あるべき状態 | 有効になっているすべての Host の上の、オンになっているすべての割り当て | `hosts`, `service_ports`, `host_service_ports` の行 |
...
| 割り当て 1 つをオフにした | 割り当ては範囲ごとそのまま残ります。そのトンネルだけが停止し、またオンにすれば復旧します |
...
**監視は keepalive の応答を監視周期のあいだ待ち、3 秒より短くは待ちません。** SSH サーバーは
その接続が運んでいるトラフィックの後ろに keepalive の応答を返すので、混んだ回線では生きている
サーバーでも応答が遅れることがあります。その時間内に応答がなければ接続を閉じて作り直します。

**失敗が続く接続は、失敗するたびに長く待ちます。** やり直す前の最初の待ち時間は監視周期で、
続けて失敗するたびに次の待ち時間が 2 倍になり、`reconnect_max_interval_sec` (既定 60 秒) で
止まります。接続が確立すると待ち時間は監視周期に戻ります。上限が監視周期より小さければ、
待ち時間はすべて監視周期です。ローカルフォワードと SOCKS5 プロキシも同じです。
...
`target_address:target_port` へ運びます。`ssh -L` がすることを、トンネルと同じように維持するものです。
...
はそうではありません。追加、変更、オンとオフの切り替え、削除は、Hosts 画面のその Host の行に
...
| `target_address` | Host から接続しに行く先。Host が引くホスト名か、IPv4 か IPv6 のアドレス |
| `target_port` | 転送先のポート。1 から 65535 |
| `description` | 自由記述 |
| `allowed_sources` | `local_port` に接続してよいアドレス。空ならすべてのアドレスを受け付けます。作成で省くと空になり、更新で省くと保存されている値のまま |
| `enabled` | 動かすかどうか。作成で省くと動くローカルフォワードになり、更新で省くと保存されている値のまま |
...
`target_address:target_port` に届きます。SSH のログインは tunnel-manager がすでに済ませているから
です。このマシンの外から使う予定がなければ `loopback` を選び、使うならそのポートの前に
ファイアウォールを置いてください。

**`allowed_sources` は、ワイルドカードのローカルフォワードに誰が接続できるかを絞ります。**
[SOCKS5 プロキシ](#host-の-socks5-プロキシ)の `socks_allowed_sources` と同じものを受け取ります。
IPv4、IPv6 のアドレスと CIDR ブロックをカンマか空白で区切って `192.0.2.0/24, 198.51.100.7`
のように書き、読めない値は `400` と `local_forward.allowed_sources.invalid` で拒否します。空なら
すべてのアドレスを受け付け、この項目ができる前に保存されたローカルフォワードはすべてそうです。
一覧にないアドレスのクライアントは、そのために何かへ接続する前に閉じ、拒否はローカルフォワード
ごとに 1 分に 1 回まで `tunnel.local_forward_source_refused` としてログに残します。一覧はどちら
の範囲でも守られますが、フォームが尋ねるのはワイルドカードのときだけで、`loopback` で保存すると
一覧は空になります。

組の 2 つのアドレスはそれぞれのアドレスファミリーで開いてみて、**片方が開けば先に進みます。**
IPv6 のないマシンは IPv4 側だけを開きます。どちらも開けないとき、たとえば別のプログラムがその
ポートを使っているときは、`error` を報告してやり直します。Windows では wildcard を両方の
アドレスファミリーを受ける `::` のソケット 1 つで開くため、別のプログラムがどのアドレスで
そのポートを使っていても `error` になります。

**`local_port` は、どの Host のものであっても、ローカルフォワードと SOCKS5 プロキシ全体で
一意です。** どれも同じこのマシンにポートを開くからです。ほかのローカルフォワードか Host の
[SOCKS5 プロキシ](#host-の-socks5-プロキシ)が使っているポートでのローカルフォワードは `409` で
拒否され、このサーバーが待ち受けるよう保存されているポート (`api_port`) と同じものも、起動が
別のポートへ移ったときに今待ち受けているポートと同じものも同様です。逆向きも同じで、設定画面の
保存や設定のインポートで `api_port` を変えるとき、ローカルフォワードか SOCKS5 プロキシが開く
ポートなら `409` で拒否し、何も保存しません。拒否にはそのローカルフォワード、または
`socks_host` にその Host と、`suggested_port` (そのポートより上で、ローカルフォワード、SOCKS5
プロキシ、保存されている `api_port`、今待ち受けているポート、求められた `api_port` のどれも
使っていない最初のポート) が載り、設定画面はローカルフォワードかプロキシを、保存のときは代わりに
API ポートを別のポートへ移すパネルを出します。1024 未満のポートはこのプロセス自身が開くので、
[非 root ユーザーで動かす](#非-root-ユーザーで動かす)の規則に従います。

**オフのローカルフォワードも `local_port` を持ち続けます。** ポートを開かず SSH 接続もしません
が、上の検査はどれも動いているものと同じように数えます。そのため、別のローカルフォワード、
SOCKS5 プロキシ、`api_port` がそのポートを使おうとすれば同じく `409` で拒否され、オンに戻した
ときにポートがほかに取られていることはありません。

**ローカルフォワードは、それ自体がオンで、その Host が有効な間だけ動きます。** オフにすると次の
調整で止まり、ポートを閉じて SSH 接続を切ります。調整ループが、トンネルと同じやり方
でローカルフォワードを起動し、組み直し、止めます。1 行が自分の SSH 接続 1 本で、Host、接続情報、
信頼しているホストキー、ポート、範囲、転送先、許可する接続元のどれかが変わると、止めてから起動し直します。
説明だけの変更では何も組み直しません。
...
接続は同じ周期のあとに、試行が失敗し続けるならもっと長く待ってから再接続します。
[トンネル 1 本の動き](#トンネル-1-本の動き)を参照してください。
...
| `status` | 意味 |
|----------|------|
| `disabled` | Host が無効です。ローカルフォワードがオンでもオフでも何も開きません |
| `off` | Host は有効で、ローカルフォワードがオフです。何も開きません |
| `stopped` | Host もローカルフォワードもオンですが、動いているローカルフォワードがありません。調整ループがまだその行に来ていないか、起動に失敗したかで、失敗ならログに出ます |
...
| `error` | 直前の試行が失敗し、`last_error` に理由があります。ログインが拒否されたときは Host を変更するまでここに留まり、それ以外は監視周期のあとにやり直し、続けて失敗するたびに待ち時間が 2 倍になって `reconnect_max_interval_sec` まで延びます |
...
と `GET /api/status` が返します。`GET /api/status` はトンネルの隣にローカルフォワードも載せ、
トンネルと合わせて数えます。[トンネルの状態を読む](#トンネルの状態を読む)を参照してください。
...
**Host は SOCKS5 プロキシを 1 つ持てます。** `ssh -D` がすることを、ローカルフォワードと同じ
ように維持するものです。**このマシンが** `socks_port` を開き、そこをプロキシに設定した
ブラウザが SOCKS5 の `CONNECT` でサイトを求めると、そのサイトへの接続は Host の SSH 接続を
通って Host から出ていきます。転送先はブラウザが接続ごとに決めるので、1 つのポートで Host が
届く先のどこにでも届きます。名前は受け取ったまま Host に渡し、Host の側で解決するので、Host
側のネットワークでしか通じない名前も使えます。
...
flowchart LR
    ...
    target[("ブラウザが求めたサイト<br/>名前は Host が解決する")]

    tm ==>|"1. SSH で接続してから socks_port を開く"| sshd
    browser -->|"2. 名前かアドレスで SOCKS5 CONNECT"| port
    port -->|"3. SSH 接続を通って"| sshd
    sshd -->|"4. 名前を解決して接続する"| target
```

**Hosts 画面の Host の追加と編集のフォームで有効にします。** **SOCKS5 プロキシを有効にする**
にチェックを入れると、ポート、開く範囲、**許可する接続元** の欄が現れ、ポートには 1080 が
入っています。API では同じ 4 つの項目を Host に載せて送ります。[Host](#host) を参照して
ください。

| 項目 | 何か |
|------|------|
| `socks_enabled` | Host がプロキシを持つかどうか。送らなければオフ |
| `socks_port` | このマシンに開くポート。1 から 65535。プロキシがオンなら必須 |
| `socks_bind_scope` | このマシンのどのアドレスに開くか。`wildcard` (`0.0.0.0` と `::`) か `loopback` (`127.0.0.1` と `::1`)。Host の登録時に与えなければ `loopback` |
| `socks_allowed_sources` | クライアントが接続してきてよいアドレス。空ならすべてのアドレスを受け付ける |

**既定は `loopback` で、プロキシはパスワードを求めません。** v3.13.6 から、
`socks_bind_scope` なしで登録した Host と Hosts 画面で追加した Host は、プロキシをこのマシンに
だけ開きます。それより前に保存された Host は、保存されている範囲のままです。プロキシは普通ほかの
マシンのブラウザが使うもので、そのためには `wildcard` を選ぶ必要があり、すると、このマシンの
`socks_port` に届く人は誰でも Host 側のネットワークに入れます。ワイルドカードで開くプロキシには **許可する接続元を入れてください。**
Chrome と Firefox には SOCKS5 プロキシにパスワードを渡す場所がないので、ログインの代わりに
クライアントの接続元アドレスでプロキシを絞ります。そのポートの前に置くファイアウォールが残りの
半分です。

`socks_allowed_sources` は IPv4 と IPv6 のアドレスと CIDR を、カンマか空白で区切って受け取り
ます。例: `203.0.113.0/24, 198.51.100.7`。アドレス 1 つはそのアドレスだけを表します。IPv6 の形で
書いた IPv4 アドレス `::ffff:192.0.2.1` は `192.0.2.1` と同じです。読めない値や、`%eth0` の
ようなゾーンの付いたアドレスは `400` で拒否し、何も保存しません。

**一覧にないアドレスからのクライアントはすぐに閉じます。** 1 バイトも読まず、SOCKS の応答も
返しません。拒否はプロキシごとに 1 分に 1 回まで `tunnel.socks_source_refused` として記録し、
前の行からあとに拒否した数を添えます。ポートを走査されてもログがあふれません。

プロキシが話すのは認証なしの SOCKS5 の `CONNECT` で、転送先は IPv4 アドレス、IPv6 アドレス、
名前です。

| クライアントが求めるもの | 返るもの |
|--------------------------|----------|
| `CONNECT` | 接続。Host が接続できなければ SOCKS の失敗応答 |
| `BIND`、`UDP ASSOCIATE` | 「サポートしないコマンド」の応答 |
| ユーザー名とパスワード、かつ認証なしの方式は受けない | 受け付けられる方式がないという応答のあと、接続を閉じる |
| SOCKS4 か SOCKS4a | 応答なしに接続を閉じる |

Host が届かない転送先には、SSH サーバーが伝えた内容に合わせて SOCKS の失敗応答を返します。
接続拒否、ネットワークやホストに届かない、SSH サーバーが許さない、それ以外は一般的な失敗です。
Host からの接続には 10 秒を与え、クライアントが要求を送るのにも 10 秒を与えます。こうした失敗は
ブラウザを使っていればいつも起きることなので、`debug` レベルでだけ記録します。

**`socks_port` はこのマシンで一意です。** プロキシを有効にするときや別のポートへ移すとき、
ほかの Host のプロキシが開くポート、どの Host のものであれローカルフォワードが開くポート、この
サーバーが待ち受けるよう保存されているポート (`api_port`)、今待ち受けているポートなら `409` で
拒否します。オンのプロキシは、Host が無効でもそのポートを押さえます。Host を有効にした瞬間に
開くからです。逆向きも同じで、ローカルフォワードと新しい `api_port` はプロキシが押さえている
ポートを使えません。[ローカルフォワード](#ローカルフォワード)を参照してください。1024 未満の
ポートは[非 root ユーザーで動かす](#非-root-ユーザーで動かす)の規則に従います。

**プロキシは、その Host が有効で**、プロキシがポートとともにオンのときに動きます。同じ Host の
トンネルやローカルフォワードとは別に自分の SSH 接続 1 本を使い、調整ループがローカル
フォワードと同じやり方で起動し、組み直し、止めます。プロキシのポート、範囲、許可する接続元か、
Host のアドレス、接続情報、信頼しているホストキーが変わると、止めてから起動し直し、プロキシを
通っていた接続も一緒に切れます。

**`socks_port` は SSH 接続がある間だけ開いています。** `local_port` と同じです。接続ができて
から開き、切れたら閉じ、監視周期のあとに再接続し、続けて失敗すればローカルフォワードと同じく長く待ちます。ログインの拒否とホストキーの拒否では、
作られた材料が変わるまで止まります。[ホストキーの承認](#ホストキーの承認)を参照してください。

状態は Host の `socks_status` に、隣の `socks_last_error` とともに載ります。Hosts 画面は
**SOCKS5 プロキシ** の列に、ポートと状態のバッジで表示します。プロキシのない Host はその列が
`-` で、エラーは行の下の行に出ます。

| `socks_status` | 意味 |
|----------------|------|
| `off` | プロキシがオフです。何も開きません |
| `disabled` | プロキシはオンで、Host が無効です。何も開きません |
| `stopped` | Host は有効ですが、動いているプロキシがありません。調整ループがまだ来ていないか、起動に失敗したかで、失敗ならログに出ます |
| `starting`、`connected`、`reconnecting`、`error`、`host_key_unapproved`、`host_key_mismatch` | 動いているプロキシが報告する値で、意味はローカルフォワードと同じです。[ローカルフォワード](#ローカルフォワード)を参照してください |

`connected` は SSH 接続があって `socks_port` が開いていることを示し、サイトが応答することは
示しません。状態はメモリにあり、`GET /api/status` には含まれず、状態画面も数えません。Host の
SSH サーバーは、ローカルフォワードと同じく、この向きの転送を許していなければなりません。

**使うには、ブラウザがこのマシンを指すようにします。** プロキシが `192.0.2.20:1080` にある
場合:

- Firefox (英語の画面の場合): **Settings**、**Network Settings**、**Manual proxy
  configuration**。**SOCKS Host** と **Port** に `192.0.2.20` と `1080` を入れて **SOCKS v5** を
  選び、**Proxy DNS when using SOCKS v5** にチェックを入れます。こうすると名前はブラウザが
  動くマシンではなく Host が解決します。
- Chrome: コマンドラインでプロキシを与えて起動します。

  ```bash
  google-chrome --proxy-server="socks5://192.0.2.20:1080"
  ```

  Chrome は `127.0.0.1` のようなループバックアドレスをプロキシに送りません。Host が自分の
  ループバックアドレスで出しているページに届くには、`--proxy-bypass-list="<-loopback>"` を
  加えます。

**エクスポートのファイルには、Host ごとにそのプロキシが載ります。** 上の 4 つの項目がその Host
に入ります。それらのないファイルは Host のプロキシをそのままにします。
[エクスポートとインポート](#エクスポートとインポート)を参照してください。
...
2. 初期パスワードは、**データベースファイルがあるディレクトリ** の `initial-password` という
   ファイルに、パーミッション `0600` で書かれます。Windows ではディレクトリの ACL を引き継がず、
   所有者・SYSTEM・Administrators だけが読める ACL で作られ、以前のリリースが残したファイルは起動時に
   その ACL に絞られます。大文字と数字で 52 文字です。
...
| 画面 | パス | 何を見せ、何をするか |
|------|------|----------------------|
| Status | `/ui/status` | 4 つの数 (あるべき数、接続中、再接続中、エラー。どれも 2 つの種類を合わせて数えます) と、その差についての 1 文、そしてどちらの種類の行も 1 行ずつです。行には Host、種類、サービスポート、状態、サーバー、開く側、到達する先、ポートに到達できたか、再試行回数、最終接続時刻が並びます。ローカルフォワードの行のサービスポートの欄は `-` で、開く側と到達する先の欄には、そのアドレスがどちらのマシンのものかも書かれます。2 つの種類はポートを開く端が互いに逆だからです。ポートに到達できたかの欄は両方の種類で埋まり、問うているものが互いに違います。トンネルでは Host の上に開いたポートのことで、ローカルフォワードでは Host から到達する相手のことです。何か問題があるトンネルは、その下に表の幅いっぱいの行で何が起きたかを載せ、転送ポートに到達できなかったトンネルは、そこに、バナーから分かった SSH サーバーの何を変えればよいか、ほかに何を確認すればよいかを載せます。確立しているトンネルは、その下にその転送のアドレスについて分かっていることを載せ、要求したこと、SSH サーバーが応答したこと、ここから開いた接続で確認したことを分けて書きます。ポートが開いているとは書きません。行は 1 ページずつ、はじめは 1 ページ 10 行で返り、表の上でサイズとページを選べます。数はページではなく常にインストール全体の数です。5 秒ごとに問い合わせ、読んでいたページのまま更新されます。表の上の検索欄で行を絞れます。[検索](#検索)を参照してください。 |
| Hosts | `/ui/hosts` | Host 1 つにつき 1 行で、ID、アドレス、ポート、ユーザー、説明、有効かどうか、SOCKS5 プロキシ、更新日時が並びます。行は 1 ページずつ、はじめは 1 ページ 10 行で返り、表の上でサイズ (10, 20, 30, 50, 100) とページを選べます。選んだ値はこの画面のものとして覚えられ、いちばん小さいサイズの 1 ページに収まる短い一覧では、操作は何も出ません。Host の追加、編集、有効化と無効化、削除ができます。追加と編集のフォームには秘密鍵を貼り付ける欄、鍵ファイルをドロップする場所、パスフレーズ付きの鍵のための欄があり、追加のフォームには既定でオンの **Assign all service ports** のチェックがあって、その Host が最初に何を担当するかを決めます。その隣の **Host での到達範囲** のリストが、そのチェックで作られる割り当てすべての出発点になる範囲です。行の **Service ports** を押すと、すべてのサービスポートの一覧がパネルで開き、この Host が担当しているものにチェックが付いていて、行ごとに到達範囲が横に並びます。上で範囲を選んでチェックしたものすべてに適用することも、1 行だけを変えることもでき、チェックしていない行はそのままです。行ごとの **有効** のチェックは、その割り当てを外さずにトンネルだけを止めます。保存では変更した分だけを送るので、その一覧で付けたチェックは、読んでいないページに影響しません。行の **Local forwards** を押すと、その Host のローカルフォワードが状態とともに 1 ページずつパネルで開き、そこで追加、変更、オンとオフの切り替え、削除を、1 行ずつでもチェックした行をまとめてでもできます。[ローカルフォワード](#ローカルフォワード)を参照してください。追加と編集のフォームでは Host の SOCKS5 プロキシも有効にでき、その列にポートと状態が出ます。[Host の SOCKS5 プロキシ](#host-の-socks5-プロキシ)を参照してください。 |
| Service Ports | `/ui/service-ports` | サービスポート 1 つにつき 1 行で、ID、サービスアドレス、サービスポート、ローカルポート、説明、更新日時が並びます。行は Hosts と同じように 1 ページずつ返り、サイズとページはこの画面専用のものです。追加、編集、削除ができます。追加のフォームには既定でオンの **Assign to all hosts** のチェックがあり、最初にどの Host が担当するかを決め、その隣の **Host での到達範囲** のリストが、そのチェックで作られる割り当ての出発点になる範囲です。そのあとどの Host が担当するか、そしてその割り当てがそれぞれどこまで届くかは Hosts 画面から変えます。Hosts と Service Ports の画面には、それぞれ表の上に検索欄があります。[検索](#検索)を参照してください。 |
...
| Settings | `/ui/settings` | 保存してあるがまだ反映されていない設定と、それを反映させる再起動ボタンをそのカードに、保存されているすべての設定と保存で何が変わったか (言語を選んでいないブラウザにこのインストールをどの言語で表示するかと、それぞれテストボタンの付いた webhook とメールの通知もここにあります。[通知](#通知)を参照してください)、提供中の証明書と更新ボタンと独自の証明書を登録する欄、このアカウントのユーザー名とパスワード、トンネル設定とこのマネージャの設定をそれぞれ暗号化した 1 ファイルに書き出すエクスポートとそれを取り込むインポート、サービスを停止して復旧させる再起動、そしていちばん下にアンインストールがあります。[設定](#設定)を参照してください。 |
...
65535 の間でなければなりません。アドレスの欄は英字、数字、ハイフン、ドット、コロンだけを
受け取り、ホスト名か IPv4、IPv6 のアドレスとして読めなければなりません。問題はその欄の隣に出て、直るまで何も
ブラウザの外には出ません。
...
| 再接続までの最長の待ち時間 (秒) | `reconnect_max_interval_sec` | `monitoring.reconnect_max_interval_sec` | `60` | 次の起動から |
...
| 宛先アドレス | `smtp_to` | `alert.smtp.to` | 空 | **保存した時点で** |
| メールサーバーの証明書を確認しない | `smtp_skip_verify` | `alert.smtp.skip_verify` | `false` | **保存した時点で** |

**保存した時点で有効になる設定は、ログレベル、言語、アップデートの 3 つの設定、そして通知の設定です。** 通知の設定は見回るたびに読み直します。[通知](#通知)を参照してください。 ログレベルは起動時に配られた
...
| 設定 | 規則 |
|------|------|
| `api_port` | 1 から 65535。ローカルフォワードか SOCKS5 プロキシが開くポートに変えると `409` で拒否されます。[ローカルフォワード](#ローカルフォワード)を参照してください |
| `monitoring_interval_sec`, `reconcile_interval_sec` | 0 より大きいこと |
| `reconnect_max_interval_sec` | 1 から 3600 |
...
| `alert_after_sec` | 10 から 86400 |
| `alert_webhook_url` | 空か、2048 文字以下の `http://` または `https://` のアドレス |
| `smtp_port` | 1 から 65535 |
| `smtp_security` | `none`、`starttls`、`tls` のいずれか |
| `smtp_auth` | `none`、`plain`、`login` のいずれか |
| `smtp_from`, `smtp_to` | メールアドレス。`smtp_to` はカンマで区切って複数受け取ります。`smtp_host` があるときはどちらも必須で、`smtp_auth` が `none` でなければ `smtp_username` も必須です |
...
システムが選んだポート、ローカルフォワードも SOCKS5 プロキシも開かないポートで待ち受け、そのことを
...
**トンネル、ローカルフォワード、SOCKS5 プロキシが `alert_after_sec` (既定 300 秒) のあいだ接続の
ないままなら通知を送り、接続し直したらもう一度送ります。** 設定は設定画面の **通知** カードにあり、
webhook とメールは別々にオンにします。webhook URL を保存すれば webhook がオンになり、メール
サーバーを書けばメールがオンになります。どちらもなければ何も見張りません。

見張るのは動いているすべての転送で、状態画面が表示するのと同じ状態を 5 秒ごとに読みます。
`connected` 以外の状態はすべて切れているものとして数えます。`starting`、`reconnecting`、`error`、
拒否されたホストキーもそうです。切断 1 回につき `down` 1 つと `up` 1 つです。オフにした、
削除した、または Host を無効にした転送は切断ではないので、`up` を送らずに忘れます。何を送ったか
はメモリにだけ置くので、再起動すると最初から数え直します。再起動のあとも切れたままの転送は時間が
過ぎればもう一度通知し、サービスが止まっていたあいだに戻った転送には `up` が届きません。通知が
オフだったあいだに始まった切断は、通知をオンにした時点から数えます。

**webhook にはこの JSON を** `Content-Type: application/json` で POST します。2xx 以外の応答は
失敗で、POST 1 回に 10 秒を与え、送り直しはしません。失敗はログに残ります。
...
```json
{
  ...
  "last_error": "<転送が最後に報告したエラー>",
  "installation": "<このマシンのホスト名>"
}
```

| 項目 | 何か |
|------|------|
| `event` | `down`、`up`、それにテストボタンが送る `test` |
| `kind` | `service_port`、`local_forward`、`socks` のいずれか |
| `host` | その転送が経由する Host のアドレス |
| `local_port` | その転送が開くポート。サービスポートは Host の上に、ローカルフォワードと SOCKS5 プロキシはこのマシンに開きます |
| `since` | 接続がないのを最初に見た時刻。UTC の RFC 3339 で、`up` は対応する `down` と同じ時刻を載せます |
| `last_error` | 切れていたあいだに見た最後のエラー |
| `installation` | このマシンのホスト名。複数のインストールが 1 か所へ送った通知を見分けるためのものです |
...
**通知をどこへ送るかは暗号化して保存します。** メールのパスワード、そして webhook URL とメール
サーバー、そのポート、ユーザー名、2 つのアドレスは、[暗号化キー](#暗号化キー)で暗号化してから
保存します。待つ時間、接続のセキュリティ、ログイン方式、証明書を確認するかどうかはそのまま保存
します。

**webhook URL とメールのパスワードは書き込み専用です。** `GET /api/settings` は値の代わりに
`alert_webhook_url_set` と `smtp_password_set` を返し、保存の応答の変更一覧でも、暗号化される
設定はすべて伏せて示します。保存でどちらかを省くか空で送ると、保存されている値はそのままで、
`alert_webhook_url_clear` か `smtp_password_clear` を true で送ると消します。

**メールの送り先を変えるときは、パスワードをもう一度求めます。** パスワードが保存されている
あいだに `smtp_host`、`smtp_port`、`smtp_username`、`smtp_security` を変える、または
`smtp_skip_verify` をオンにする保存やテストは、`smtp_password` を一緒に送らなければならず、
送らなければ `400` と `settings.smtp_password.required` で拒否します。そうしないと、リクエストの
本文が別のサーバーを書いて、保存されているパスワードをそこへ送らせることができてしまいます。
そうした本文がログインをオフにするかパスワードを消すなら、保存されているパスワードを代わりに
消します。

**2 つのテストボタンは `test` の通知をすぐに送ります。** `POST /api/settings/alert/test-webhook`
と `POST /api/settings/alert/test-smtp` です。`PUT /api/settings` と同じ本文を受け取って保存
されている設定の上に重ねるので、画面にある値を保存する前に試せ、何も保存しません。webhook か
メールサーバーが通知を受け取らなければ `502` を返し、理由を `error_args.reason` に載せます。
...
設定のエクスポートは通知の設定を、webhook URL とメールのパスワードも含めて、暗号化された
ファイルの中に平文で入れます。Host のパスワードを入れるのと同じです。
...
画面にまたアクセスできます。既定の API ポートを開くローカルフォワードか SOCKS5 プロキシが
あると次の起動が別のポートで待ち受けることがあるため、それを警告
（`settings.default_port_held_by_forward` か `settings.default_port_held_by_socks`）で知らせます。
...
| `400 The field ip was renamed to address. Send address instead` | 本文が v3.14.0 より前の名前 (`ip`、`service_ip`、`target_ip`) で項目を書いていて、`error_code` は `request.field_renamed` です。何も保存されていません。 |
...
`GET /api/host/:id/local-forward` もその Host のローカルフォワードすべてをそうした配列で返して
いましたが、今は `page` と `size` を受け取り、この形で応答します。
[Host のローカルフォワード](#host-のローカルフォワード)を参照してください。

`/api/status` はもともとオブジェクトでした。`tunnels` が 2 つの種類を 1 つの一覧に収めた状態
の行の 1 ページになり、その隣に `page` と `size` が並びます。そして **数はページではなく全行に
わたるもの** です。数が示すのはインストールが何をしているかであって、いま見ているページの
ことではありません。

```bash
# Host を 20 件ずつにした 2 ページ目と、状態の行の最後のページ。最後より後ろのページは
...
**`q` は同じ 3 つの一覧を、その文字列を含む行に絞ります。** `q` がないか空なら絞りません。ASCII
の文字は大文字と小文字を区別せず、ポートは書かれた文字列として照らし合わせるので `22` は `2222`
番の Host も見つけ、`%` と `_` はワイルドカードではなく書かれたとおりに扱います。Hosts、
Service Ports、状態画面の表の上にある検索欄がこれを送ります。

| 一覧 | `q` を探す場所 |
|------|----------------|
| `GET /api/host` | アドレス、ユーザー、説明、SSH ポート |
| `GET /api/service-port` | サービスアドレス、サービスポート、ローカルポート、説明 |
| `GET /api/status` | Host のアドレスと説明、そしてその行の開く側と到達する先のアドレス |

ページは一致した行から切り出すので、前の 2 つの `total` と状態の `total_rows` は一致した行の数
です。`GET /api/status` の 4 つの数は、これまでどおりインストール全体を数えます。
...
| `GET` | `/api/host` | Host の 1 ページを、古い順に返します。`page` と `size` を受け取ります。[ページング](#ページング)を参照してください。`q` も受け取ります。[検索](#検索)を参照してください |
...
| `GET` | `/api/host/:id/local-forward` | この Host のローカルフォワードの 1 ページを状態付きで返します。[Host のローカルフォワード](#host-のローカルフォワード)を参照してください |
...
| 項目 | 作成のとき | 更新のとき |
|------|------------|------------|
| `address`, `port`, `user` | 必須。`address` はホスト名か IP アドレス | 省略可。省いたものはそのまま残ります |
...
| `socks_enabled`, `socks_port` | 省略可。`socks_enabled` が true なら `socks_port` は必須です | 省略可。省いたものはそのまま残ります |
| `socks_bind_scope` | 省略可。`loopback` か `wildcard` で、省くか空で送ると `loopback` です | 省略可。省くか空で送ると、保存されている範囲がそのまま残ります |
| `socks_allowed_sources` | 省略可。空ならすべてのアドレスを受け付けます | 省略可。省くと保存されている一覧がそのまま残り、`""` で送るとすべてのアドレスを受け付けます |
...
`socks_bind_scope` はこれとは別のもので、Host の SOCKS5 プロキシをこのマシンのどこに開くかです。
...
**SOCKS5 の項目は、Host のプロキシを組み立てる材料です。**
[Host の SOCKS5 プロキシ](#host-の-socks5-プロキシ)を参照してください。ポートなしで有効にしたり、
許可する接続元が読めなかったりすると `400` で拒否します。ほかの Host のプロキシ、ローカル
フォワード、このサーバーが押さえているポートで有効にしたり移したりすると `409` で拒否します。
どちらの場合も何も保存しません。Host を載せる応答には、保存されている 4 つの項目、常に語で書かれた
`socks_bind_scope`、そして `socks_status` と `socks_last_error` が載ります。

```bash
# Host 3 の SOCKS5 プロキシをポート 1080 で有効にし、2 つの接続元だけを受け付ける。
...
範囲がなく、画面がそこから作る行は、ほかを選ぶまでワイルドカードから出発します。`enabled` は
その割り当てのトンネルが動くかどうかで、この Host が担当していない行では false です。
...
| `switch` | `enabled` へ切り替える割り当てで、同じように id で指定します。この Host が担当していない id は何も切り替えず、`enabled` を省いたリクエストも何も切り替えません |
| `enabled` | `add` で書かれる行を動かすかどうか、そして `switch` が指した行を何に切り替えるか。省くと `add` の行は動き、何も切り替えません |
...
**割り当ては外さずにオフにできます。** オフにした割り当ては範囲ごと割り当てられたまま残り、
トンネルだけが動きません。またオンにすれば同じトンネルが戻ります。Host の **Service ports**
パネルで行ごとにある **有効** のチェックが、画面で同じことをします。範囲と同じく `switch` が
名指しした行だけが変わるので、すでに入っていたチェックをもう一度入れても、誰かが止めておいた
トンネルが動き出すことはありません。

```bash
# Host 1 でサービスポート 3 を外さずに止める。
curl -s -b cookies.txt -X PUT "$BASE/api/host/1/service-port" \
  -H 'Content-Type: application/json' \
  -H "X-CSRF-Token: $CSRF" \
...
`added`, `removed`, `rescoped`, `switched` が数えるのは行であって、リクエストの中身ではありません。
すでに割り当ててあるサービスポートをもう一度指定しても行は書かれず、割り当てていないものを外しても
行は消えず、この Host が担当していない割り当てを `rescope` や `switch` に書いても変わる行はありま
せん。4 つのリストはいずれも省略でき、何も変えないリクエストも拒否せずに応答します。
...
これが、そのあとで `GET /api/host/1/local-forward` が返すものです。1 ページで、ページングする
ほかの一覧と同じ形です。[ページング](#ページング)を参照してください。作成、読み出し、更新は、
`items` の中と同じオブジェクト 1 つで応答します。応答の `bind_scope` は常に `loopback` か
`wildcard` で、空になることはありません。

**この一覧は以前はすべての行を返し、`data` がその配列でした。** 今はページングされ、`data` は
上のオブジェクトなので、`data[0]` を読んでいたクライアントは `data.items[0]` を読みます。

作成と更新の本文は同じ項目を受け取り、更新はそのすべてを受け取ります。`local_port`、
`target_address`、`target_port` はどちらでも必須で、**更新で `bind_scope` を省くとワイルドカードに
なります。** 今の範囲のままにするには、その範囲を送ってください。`enabled` と `allowed_sources` は、更新で
省いてもそのまま残る 2 つの項目です。省くとオンでもオフでも今のままで一覧も残り、作成で省くと
すべてのアドレスを受け付ける、動くローカルフォワードになります。
...
| 送ったもの | どうなるか |
|------------|------------|
| 1 から 65535 の外のポート、ホスト名でも IP アドレスでもない `target_address`、`loopback` でも `wildcard` でもない `bind_scope` | `400`。何も書き込みません |
| 読めない `allowed_sources` | `400`、`local_forward.allowed_sources.invalid`。何も書き込みません |
| `target_address` の代わりの `target_ip` | `400`、`request.field_renamed`。何も書き込みません |
...
| `GET` | `/api/service-port` | サービスポートの 1 ページを、古い順に返します。`page` と `size` を受け取ります。[ページング](#ページング)を参照してください。`q` も受け取ります。[検索](#検索)を参照してください |
...
| メソッド | パス | 何をするか |
|----------|------|------------|
| `GET` | `/api/status` | インストール全体の数と、状態の行の 1 ページを返します。サービスポートのトンネルとローカルフォワードが、それぞれ `kind` を付けて一緒に載ります。`page` と `size` を受け取ります。[ページング](#ページング)を参照してください。`q` も受け取ります。[検索](#検索)を参照してください |
...
| `POST` | `/api/settings/alert/test-webhook` | ボディの設定を保存されているものの上に重ね、webhook に `test` の通知を送ります。何も保存しません。[通知](#通知)を参照してください |
| `POST` | `/api/settings/alert/test-smtp` | 同じやり方で `test` の通知をメールで送ります |
...
**割り当てごとに動くかどうかも一緒に運ばれます。** Host 側の `assigned_enabled` がそれで、同じ
ローカルポートをキーにします。オフの割り当てだけが `false` として書かれ、項目のない割り当ては
動くので、割り当てをオフにできるようになる前のファイルは、すべての割り当てをオンのままインポート
します。
...
一覧のローカルフォワードにはそれぞれ `enabled` が載り、エクスポートは必ずそれを書きます。
`enabled` のないローカルフォワード、つまりローカルフォワードをオフにできるようになる前のファイル
のものは、すべてオンで保存されます。そのファイルを作った側では、どれも動いていたからです。
ローカルフォワードごとに `allowed_sources` も載り、それがないローカルフォワードは、この項目が
できる前のものがすべてそうだったように、すべてのアドレスを受け付けます。
...
**Host ごとにその SOCKS5 プロキシも一緒に移ります。** Host の `socks_enabled`、`socks_port`、
`socks_bind_scope`、`socks_allowed_sources` がそれで、エクスポートは 4 つを常に書きます。ファイル
にない項目は Host が持つ値をそのままにするので、プロキシができる前に作ったファイルはどの
プロキシもオフにしません。スキップされた Host は、ファイルが何と言っていてもプロキシをそのまま
持ち続けます。インポートが有効にするプロキシは、応答に種類 `socks` として載ります。**ファイルが
プロキシとローカルフォワードの間で同じポートを 2 回開くとき、ファイルが有効にするプロキシが、
このサーバーが待ち受けるポートか、ここにあるほかの Host のプロキシかローカルフォワードが開く
ポートを開くときは、インポートは拒否され、何も書き込まれません。**
...
サービスポートとその間の割り当てから、調整ループが作るものだからです。英語の文である `reason` と
`name` には、`reason_code` と `reason_values`、`name_code` と `name_values` が付きます。画面はこのコードと
値から、その文を自分の言語で書きます。
...
ファイルは拒否し、何も保存しません。`api_port` がここでローカルフォワードの開くポートに変わる
ファイルも、保存と同じく `409` で拒否し、何も保存しません。
...
**`local` と `remote` は 2 つの種類の間で意味が反転するので、`kind` と一緒に読んでください。**
サービスポートの行では、`local` は **Host** に開くアドレスで、`remote` はこちらが代わりに
接続するサービスです。ローカルフォワードの行では逆に、`local` が **このマシン** に開くアドレス
で、`remote` が Host から接続する相手です。どちらもアドレスとポートなので見た目は同じです。
上の例では `0.0.0.0:18080` と `127.0.0.1:15432` で、どちらのマシンの話なのかを示すものは
...
`sp_id` があればトンネルの行です。`forward_reach` は両方の種類に載り、問うているものが互いに
違います。それは[ローカルフォワードがターゲットに届くか](#ローカルフォワードがターゲットに届くか)
にあります。`server_banner`, `error_kind`, `open_reach`, `listen_addresses` はサービスポートの
行にだけ埋まり、ローカルフォワードの行では空のままです。ローカルフォワードは Host の上にポートを
開かないので、これらが語る相手がありません。

数はページではなく全行にわたるものです。行が 25 あるインストールは、1 ページ 10 行でも 25 と
答え、`connected_tunnels` が数えるのはインストール全体の接続中のトンネルであって、たまたま
そのページに載っているものではありません。

**数は 4 つあり、4 つとも 2 つの種類を合わせて数えます。** この答えを読む側から見れば
ローカルフォワードもトンネルなので、4 つの数は、このインストールのどれだけが動いていて、どれだけ
が自力で戻ってくる途中で、どれだけが人を待っているのかを、種類に関わらず示します。

| 数 | 何を数えるか |
|----|--------------|
| `desired_tunnels` | 2 つの種類を合わせて、動作している **べき** 行の数です。Host が有効になっている割り当てと、オンになっていて Host が有効なローカルフォワードです |
| `connected_tunnels` | `connected` の行の数です |
| `reconnecting_tunnels` | `reconnecting` の行の数です。誰も手を出さなくても戻ってくる途中のものです |
| `error_tunnels` | `error` の行の数です。人を待っているものです |
| `total_rows` | 2 つの種類の行を足した数で、ページはこれを元に切り出されます |

**`reconnecting` と `error` を分けて数えるのは、人がすることが違うからです。** 再接続中の行は
自力で戻ってくる途中で、エラーの行は人を待っています。2 つを 1 つの数にまとめると、いま見ている
のがどちらなのかを何も言わなくなります。

**4 つの数は足し合わせても合いませんし、合うようには作っていません。** `starting` の行と、
ホストキーで止まっている行は、状態の 3 つの数のどれにも入りません。後者は下の Host の数 2 つが
答えます。次の数は、行が 6 つのインストールから返ってきたものです。1 つが再接続中、2 つが
エラーです。
...
これらの数の間で見る価値のある比較は 3 つです。

| 比較 | 何を意味するか |
|------|----------------|
| `desired_tunnels` が `connected_tunnels` より大きい | 転送しているべきものが転送していません。そのうちどれだけが自力で戻ってくる途中で、どれだけが人を待っているのかは `reconnecting_tunnels` と `error_tunnels` が示し、理由はそれぞれの行の `status` と `last_error` にあります |
| `error_tunnels` が 0 より大きい | 放っておいても直らない行があります。拒否されたログインと拒否されたホストキーは、それを組み立てている何かが変わるまでそこに留まります。手を入れる対象を数えているのがこの数です |
| `total_rows` が `desired_tunnels` より大きい | 誰も動かすことを望んでいない行が保存されています。オフにしたローカルフォワードは `off`、無効な Host のローカルフォワードは `disabled` として行に残り、`total_rows` 以外のどの数にも入りません。無効な Host のトンネルの行は残りません。調整パスが消すので `total_rows` からも外れます |

**v3.13.0 が返していた数が 4 つなくなり、名前が残った 2 つは数える対象が広がりました。**
`total_tunnels`, `total_local_forwards`, `connected_local_forwards`,
`desired_local_forwards` は 1 リリースの間だけこの答えにあり、もうありません。サービスポートの
トンネルだけを数えていた `desired_tunnels` と `connected_tunnels` は、いまはローカルフォワードも
一緒に数えます。この 2 つを読んでいたスクリプトは以前より大きい数を読むことになり、なくなった
4 つを読んでいたスクリプトは何も読めません。`GET /api/status/:hostId` は別の呼び出しで、1 台の
Host のトンネルを単位にした `total_tunnels` と `connected_tunnels` をこれまでどおり返します。
...
この 2 つもページではなくインストール全体を数えます。前の数と同じ理由に、もう 1 つ理由が
...
ここに並ぶのは **サービスポート** の行が持つ値です。ローカルフォワードの行は、ローカル
フォワードが報告する値を持ち、その中には `disabled`, `off`, `stopped` もあります。
[ローカルフォワード](#ローカルフォワード)を参照してください。
...
`forward_reach` はローカルフォワードの行にも載り、**2 つの種類は同じ問いを立てていません。**
サービスポートの行は、Host が開いたポートがここから開いた接続に応答するかを問います。
ローカルフォワードの行は、ターゲットが **Host から** 開いた接続に応答するかを問います。その
フォワードにつなぐクライアントが Host に頼むのは、まさにそれです。

| | サービスポートの行 | ローカルフォワードの行 |
|---|--------------------|------------------------|
| 何を問うか | Host に開いたポートが、ここから開いた接続に応答するか | ターゲットが、Host から開いた接続に応答するか |
| 誰が接続するか | このプロセスで、フォワードのどちらの端でもないマシンからです | Host で、そのフォワードがトラフィックを運んでいる SSH 接続の上からです |
| 沈黙を何と書くか | こちらが応答を見られる位置になければ `unknown`、見られる位置であれば `unreachable` | `unreachable` |

**ローカルフォワードは沈黙を `unreachable` と書きます。トンネルは同じ沈黙を `unknown` と
書きます。** 測り方は、Host にターゲットへ接続させることで、その接続が通るのは、このフォワード
のクライアントが運ばれるのとまったく同じ接続です。そこで返ってくるものは、少しあとにクライアント
が告げられることであり、ここでは沈黙が意味しうる別のものがありません。トンネルは、フォワードの
どちらの端でもない第三の位置、つまりこのマシンから測り、ポートが開かれたかもしれないアドレスを
すべて試すこともできないので、そこでの沈黙はこちらが応答を見られる位置になかっただけかもしれま
せん。2 つを分けているのはそのためです。

ローカルフォワードの `connected` は、SSH 接続が確立していてローカルのポートが開いていることを
言うだけで、ターゲットについては何も言いません。ターゲットに答えるのは `forward_reach` です。
`connected` でありながら `unreachable` の行は、何も運べていないフォワードです。ポートはここに
開いていて、そこに届いた接続は Host がすべて閉じます。
...
測る時点は、フォワードが接続した直後の 1 回と、再接続のたびで、トンネルと同じです。接続が
確立すると値は `unknown` に戻り、その接続を測るまでそのままなので、`unknown` はいま接続した
ばかりのフォワードであって、測った結果ではありません。切れて再接続している
フォワードは、次の接続が確立するまで前の接続で測った値を載せます。何も動いていない行には値が
ありません。オフにしたフォワードと、無効な Host のフォワードは、動いていたときの値を見せる
のではなく空になります。

`GET /api/status/:hostId` は、その Host を単位にした `total_tunnels` と
`connected_tunnels`、そして Host そのものを返します。この 2 つはその Host のトンネルだけを
数え、ローカルフォワードは数えません。この呼び出しはローカルフォワードを載せてもいません。
2 つの種類を合わせて数える数は `GET /api/status` の 4 つです。`desired_tunnels` も、ホストキー
の数 2 つも持ちません。
ページングはせず、その Host のトンネルをすべて載せます。

## 暗号化キー

Host の SSH パスワードは、保存する前に AES-256-GCM で暗号化します。秘密鍵とそのパスフレーズ、証明書の鍵、メールのパスワード、通知をどこへ送るかの設定 ([通知](#通知)を参照) も同じです。キーは **Encryption key
...
権限を絞ってから起動し直してください。Windows ではキーファイルを作るときと読み込むときに、所有者・
SYSTEM・Administrators だけが読めるように ACL を合わせます。データベースファイル・ログファイルと、
このプログラムがそれらのために作るディレクトリも同じです。
