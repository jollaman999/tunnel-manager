...
| 登録するもの | 項目 | 何か |
|--------------|------|------|
| Host | `ip`, `port`, `user`, `private_key`, `key_passphrase`, `password`, `bind_address`, `description`, `enabled` | Tunnel Manager がログインする SSH サーバーです。秘密鍵でも、パスワードでも、その両方でもログインできますが、どちらか一方は必ず要ります。鍵も、そのパスフレーズも、パスワードも、すべて暗号化して保存します。`bind_address` は転送されたポートをこの Host のどのアドレスで開くよう要求するかで、与えなければ `0.0.0.0` です。 |
...
sequenceDiagram
    ...
    rect rgb(255, 255, 220)
        ...
        Note right of Bastion: 割り当てられたサービスポートごとに: -R bindAddress:localPort:remoteIP:remotePort
    end
...
要求するアドレスは Host の `bind_address` で、与えなければ `0.0.0.0` です。
[Host](#host)を参照してください。Host 側の待ち受けが本当にそのアドレスに
開くかどうかは、Host の SSH サーバー次第です。その `GatewayPorts` が無効だと、どのアドレスを
要求しても待ち受けはループバックにバインドされ、そのことがログに出ます。トンネルが確立した
...
| メソッド | パス | 何をするか |
|----------|------|------------|
| `POST` | `/api/host` | Host を作ります。`enabled` は省略でき、指定のない Host は有効になります。`bind_address` も省略でき、省くと `0.0.0.0` です |
...
| `bind_address` | 省略可。省くか空で送ると `0.0.0.0` です | 省略可。省いたものはそのまま残ります。ワイルドカードに戻すには `0.0.0.0` を送ります |
...
**`bind_address` は、転送されたポートをこの Host のどのアドレスで開くよう要求するかです。**
受け取るのは IPv4 か IPv6 のアドレスだけです。この文字列はそのまま Host の sshd に渡るので、
名前を書けば向こう側の、こちらからは見えないリゾルバが解決することになり、解決できない名前は
拒否された接続として 1 本ずつ現れるだけです。省くか空で送ると `0.0.0.0` で、これはこの項目が
できる前にすべての転送が要求していた値なので、以前のバージョンで保存された Host の届く範囲は
変わりません。

サービスポートではなく Host が持つのは、このアドレスがその機械のインターフェースを指すから
です。1 つのサービスポートを 3 台の Host が担当すれば、待ち受けは別々の機械に 3 つできますが、
そのうち 1 台だけが外に向いたインターフェースを持っていることもあります。サービスポートに
置くと、3 台への答えが 1 つに強いられます。

画面は `0.0.0.0`, `127.0.0.1`, `::1` を提示し、それ以外のアドレスは直接入力を受け取ります。
一覧を Host に問い合わせることはしません。どのインターフェースに付いたアドレスかはその機械に
ついての事実で、それを読むには向こうでコマンドを実行する必要がありますが、このプログラムには
その手段がありません。

そのアドレスで何が起きるかを決めるのは、やはり SSH サーバーです。`127.0.0.1` は
`GatewayPorts` が何であってもポートを Host 自身にとどめ、`0.0.0.0` は Host に届くものすべてに
ポートを開きますが、それもサーバーが許す場合だけです。
[転送ポートに到達できるかどうか](#転送ポートに到達できるかどうか)を参照してください。
ループバックにバインドした Host のトンネルはそこで `unreachable` と報告されますが、これは
測定が正しく働いた結果です - その測定はこのプロセスから Host へ接続してみるものであり、
ループバックに開いたポートはそれに応答しません。

アドレスを変えると、その Host のトンネルはすべて作り直されます。アドレスはトンネルを
組み立てる値の一部だからです。
...
`<バインドアドレス>:<ローカルポート>` で、Host が別のアドレスを指定していなければ
`0.0.0.0` です。OpenSSH が既定の `GatewayPorts no` のままだったり、Dropbear が `-a` なしで
起動していたりすると、サーバーはループバックにだけバインドします。転送ポートは
