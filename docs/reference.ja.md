...
それについてわかっていることは、トンネルの行の 3 つの項目が示します。
...
三つとも測るのはトンネルが確立したときと、再接続のたびの 1 回ずつで、状態を読むたびではありません。
