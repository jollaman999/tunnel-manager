...
別の名前の数として数えます。[トンネルの状態を読む](#トンネルの状態を読む)を参照してください。
...
| 画面 | パス | 何を見せ、何をするか |
|------|------|----------------------|
| Status | `/ui/status` | 2 つの種類の数 (あるべき数、行の数、接続中の数を、トンネルとローカルフォワードのそれぞれについて) と、その差についての 1 文、そしてどちらの種類の行も 1 行ずつです。行には Host、種類、サービスポート、状態、サーバー、開く側、到達する先、ポートに到達できたか、再試行回数、最終接続時刻が並びます。ローカルフォワードの行のサービスポートの欄は `-` で、開く側と到達する先の欄には、そのアドレスがどちらのマシンのものかも書かれます。2 つの種類はポートを開く端が互いに逆だからです。何か問題があるトンネルは、その下に表の幅いっぱいの行で何が起きたかを載せ、転送ポートに到達できなかったトンネルは、そこに、バナーから分かった SSH サーバーの何を変えればよいか、ほかに何を確認すればよいかを載せます。確立しているトンネルは、その下にその転送のアドレスについて分かっていることを載せ、要求したこと、SSH サーバーが応答したこと、ここから開いた接続で確認したことを分けて書きます。ポートが開いているとは書きません。行は 1 ページずつ、はじめは 1 ページ 10 行で返り、表の上でサイズとページを選べます。数はページではなく常にインストール全体の数です。5 秒ごとに問い合わせ、読んでいたページのまま更新されます。 |
...
**トンネルの 3 つの数は、以前と変わらずサービスポートのトンネルだけを数えます。**
ローカルフォワードはその隣に、別の名前の 3 つの数として並びます。名前はそのままで数える対象
だけが広がった数は、この答えを読むスクリプトが以前の意味のまま読み続けて、気づかないうちに
間違える元になるからです。
...
| `desired_local_forwards` | 動作している **べき** ローカルフォワードの数です。オンになっていて、Host があり、その Host が有効なものを同じやり方で数えます |
| `total_local_forwards` | **保存されている** ローカルフォワードの数です。オフにしてあるものも入ります |
| `connected_local_forwards` | そのうち `connected` を報告しているものの数です |
| `total_rows` | 2 つの種類を足した数で、ページはこれを元に切り出されます |

トンネルの 3 つの数は 3 つの別々の問いに答えていて、その間の差が意味するものも違います。
...
**ローカルフォワードの側で見る差は 2 つではなく 1 つです。** `total_local_forwards` は開始
されたものではなく保存されている行を数えるので、`desired_local_forwards` を下回ることは
ありません。オフにしてあるフォワードは総数には入り、あるべき数には入らないからです。意味の
ある差は `desired_local_forwards` が `connected_local_forwards` を上回っているときで、動いて
いるべきフォワードが転送していないということです。理由はトンネルと同じく、その行の `status`
と `last_error` にあります。
