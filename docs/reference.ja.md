...
から開き、切れたら閉じ、監視周期のあとに再接続します。ログインの拒否とホストキーの拒否では、
...
なります。** 今の範囲のままにするには、その範囲を送ってください。`enabled` は、更新で省いても
そのまま残るただ 1 つの項目です。省くとオンでもオフでも今のままで、作成で省くと動くローカル
フォワードになります。
