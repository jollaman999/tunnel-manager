...
| `socks_bind_scope` | このマシンのどのアドレスに開くか。`wildcard` (`0.0.0.0` と `::`) か `loopback` (`127.0.0.1` と `::1`)。与えなければワイルドカード |
| `socks_allowed_sources` | クライアントが接続してきてよいアドレス。空ならすべてのアドレスを受け付ける |

**既定はワイルドカードで、プロキシはパスワードを求めません。** プロキシは普通ほかのマシンの
ブラウザが使うもので、`loopback` で開くと役に立たないため、ほかを選ばなければすべての
インターフェースで開きます。すると、このマシンの `socks_port` に届く人は誰でも Host 側の
ネットワークに入れます。ワイルドカードで開くプロキシには **許可する接続元を入れてください。**
...
| `socks_bind_scope` | 省略可。`loopback` か `wildcard` で、省くか空で送るとワイルドカードです | 省略可。省くか空で送ると、保存されている範囲がそのまま残ります |
