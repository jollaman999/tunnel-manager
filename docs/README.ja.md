...
ポートフォワードは普通、一度設定したらそれきりです。`ssh -R`、`-L`、`-D` のコマンドを
...
- **コマンドの寄せ集めではなく記録。** Host、サービスポート、割り当てはデータベースの行です。
  ...
  続けて失敗するたびに待ち時間を倍にし、上限で止めます。1 つのステータス画面に、あるべき数、
  接続中の数、エラーの数が出ます。
- **開く範囲をフォワードごとに選び、実際に測ります。** 割り当てとローカルフォワードごとに、
  ポートをすべてのインターフェースで開くかループバックだけで開くかを選びます。すべての
  インターフェースで開くトンネルは、確立したあと tunnel-manager が転送ポートに自分で接続して
  みて、応答したかどうかを示します。どこにバインドするかを最終的に決めるのは SSH サーバー
  だからです (OpenSSH の `GatewayPorts`)。SOCKS5 プロキシは、すべてのインターフェースを
  選ばない限りループバックで開きます。プロキシとローカルフォワードは、接続を受けるクライアントの
  アドレスをリストで制限できます。
- **アラート。** トンネル、ローカルフォワード、プロキシが設定した時間より長く切れたままなら
  Webhook かメールで知らせ、つながり直したらもう一度知らせます。
- **設定は 1 か所に。** 設定はデータベースにあり、ブラウザから変更します。設定一式を
  パスワードで封じた 1 つのファイルとして移せます。SSH のパスワード、秘密鍵、Webhook と
  メールの設定は、このインストールのキーファイルで暗号化して保存します。
