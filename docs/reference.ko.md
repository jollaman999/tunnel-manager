...
끄면 그 포트는 HTTP 만 받습니다. 인증서도 없고 리다이렉트도 없으며, 화면이 보내는 모든 것이
...
**윈도우에는 exec 가 없습니다.** 거기서는 정상 종료까지가 전부고, 다시 시작하는 것은 이
...
때문입니다. 클라이언트가 보고 있던 페이지가 다시 조회할 때는 없을 수 있고, 거기서 오류를 내면
...
목적입니다. 데이터베이스는 이 셋을 저장한 장비의 암호화 키로 암호화해 두므로, 암호화된 그대로 담은
파일은 다른 설치에서 열리지 않습니다. 그래서 내보낼 때 복호화하고, 가져간 설치의 암호화 키로
다시 암호화해서 저장합니다. 그 사이에 이것들을 지키는 것은 **파일 전체를 암호화한 비밀번호뿐**입니다.
