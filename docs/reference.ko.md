...
싣고 이름이 다른 숫자로 따로 셉니다. [터널 상태 읽기](#터널-상태-읽기)를 보십시오.
...
|------|------|------------------------------|
| 상태 | `/ui/status` | 두 종류의 숫자(목표, 행, 연결됨을 터널과 로컬 포워딩 각각)와 그 차이를 설명하는 한 줄, 그리고 두 종류의 행마다 한 줄씩 Host, 종류, 서비스 포트, 상태, 서버, 여는 곳, 닿는 곳, 포트 도달 여부, 재시도 횟수, 마지막 연결 시각. 로컬 포워딩 행의 서비스 포트 칸은 `-` 이고, 여는 곳과 닿는 곳 칸은 그 주소가 어느 장비의 것인지를 같이 적습니다. 두 종류가 포트를 여는 끝이 서로 반대이기 때문입니다. 무언가 잘못된 터널은 그 아래에 표 전체 폭으로 무엇이 잘못됐는지가 한 줄 붙고, 포워딩된 포트에 접속하지 못한 터널은 그 자리에 어느 SSH 서버에서 무엇을 고쳐야 하는지와 그 밖에 확인할 것이 붙습니다. 연결된 터널은 그 아래에 그 포워딩의 주소에 대해 아는 것이 붙는데, 요청한 것과 SSH 서버가 응답한 것과 여기서 연 연결로 확인한 것을 갈라서 적습니다. 포트가 열렸다고는 쓰지 않습니다. 행은 한 번에 한 페이지씩 나오고 처음에는 10줄이며, 표 위에서 페이지 크기와 페이지를 고릅니다. 숫자는 페이지가 아니라 설치본 전체를 셉니다. 5초마다 다시 조회하고, 보고 있던 페이지를 그대로 유지합니다. |
...
**터널 숫자 셋은 예전 그대로 서비스 포트 터널만 셉니다.** 로컬 포워딩은 그 옆에 이름이 다른
숫자 셋으로 따로 붙습니다. 이름은 그대로인데 세는 대상만 넓어진 숫자는, 이 응답을 읽는
스크립트가 계속 예전 뜻으로 읽으면서 조용히 틀리게 만듭니다.
...
| `desired_local_forwards` | **동작해야 할** 로컬 포워딩 수. 켜져 있고 Host 가 있으며 그 Host 가 활성인 것을 같은 방식으로 셉니다 |
| `total_local_forwards` | **저장된** 로컬 포워딩 수. 꺼 둔 것도 들어갑니다 |
| `connected_local_forwards` | 그중 `connected` 를 알리는 것의 수 |
| `total_rows` | 두 종류를 더한 수. 페이지는 이 수를 기준으로 자릅니다 |

터널 숫자 셋은 서로 다른 세 질문에 답하고, 그 사이의 차이도 뜻이 다릅니다.
...
**로컬 포워딩 쪽은 볼 차이가 둘이 아니라 하나입니다.** `total_local_forwards` 는 시작된 것이
아니라 저장된 행을 세므로 `desired_local_forwards` 보다 작아지지 않습니다. 꺼 둔 포워딩은
전체에는 들어가고 목표에는 안 들어갑니다. 뜻이 있는 차이는 `desired_local_forwards` 가
`connected_local_forwards` 보다 큰 경우입니다. 돌아야 할 포워딩이 트래픽을 전달하지 못하고
있다는 뜻이고, 이유는 터널과 마찬가지로 그 행의 `status` 와 `last_error` 에 있습니다.
