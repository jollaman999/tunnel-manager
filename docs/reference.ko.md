...
2. 임시 비밀번호는 **데이터베이스 파일이 있는 디렉터리**의 `initial-password` 파일에 권한
   ...
   만 읽을 수 있는 ACL 로 만들어집니다. 대문자와 숫자 52자입니다.
