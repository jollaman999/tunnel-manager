...
| 등록하는 것 | 항목 | 무엇인가 |
|-------------|------|----------|
| Host | `ip`, `port`, `user`, `private_key`, `key_passphrase`, `password`, `bind_address`, `description`, `enabled` | Tunnel Manager 가 접속하는 SSH 서버입니다. 개인키로 접속할 수도, 비밀번호로 접속할 수도, 둘 다 등록할 수도 있으며 둘 중 하나는 반드시 있어야 합니다. 키와 키 암호와 비밀번호는 모두 암호화해서 저장합니다. `bind_address` 는 포워딩된 포트를 이 Host 의 어느 주소에 열도록 요청할지이고, 안 주면 `0.0.0.0` 입니다. |
...
요청하는 주소는 Host 의 `bind_address` 이고, 안 주면 `0.0.0.0` 입니다.
[Host 관리](#host-관리)를 보십시오. Host 쪽 리스너가 정말 그 주소로 열리는지는
...
| 메서드 | 경로 | 하는 일 |
|--------|------|---------|
| `POST` | `/api/host` | Host 생성. `enabled` 는 선택이고, 안 보내면 활성으로 만듭니다. `bind_address` 도 선택이며 안 보내면 `0.0.0.0` 입니다 |
...
| `bind_address` | 선택. 빼거나 빈 값으로 보내면 `0.0.0.0` 입니다 | 선택. 빠뜨린 것은 그대로 둡니다. 와일드카드로 되돌리려면 `0.0.0.0` 을 보냅니다 |
...
**`bind_address` 는 포워딩된 포트를 이 Host 의 어느 주소에 열도록 요청할지입니다.** IPv4 나
IPv6 주소만 받습니다. 이 문자열은 그대로 Host 의 sshd 에 넘어가므로, 이름을 적으면 여기서는
볼 수 없는 리졸버가 저쪽에서 풀고, 풀리지 않는 이름은 거절된 접속 하나하나로만 드러납니다.
빼거나 빈 값으로 보내면 `0.0.0.0` 이고, 이것이 이 필드가 생기기 전 모든 포워딩이 요청하던
값이라 이전 버전에서 저장된 Host 의 도달 범위는 그대로입니다.

서비스 포트가 아니라 Host 가 들고 있는 이유는, 이 주소가 그 기계의 인터페이스를 가리키기
때문입니다. 서비스 포트 하나를 Host 셋이 담당하면 서로 다른 기계에 리스너가 셋 생기고, 그중
한 대만 외부에 노출된 인터페이스를 가지고 있을 수 있습니다. 서비스 포트에 두면 셋에 대한
답이 하나로 강제됩니다.
...
[포워딩된 포트에 접속되는지](#포워딩된-포트에-접속되는지)를 보십시오. 루프백에 묶은 Host 의
터널은 거기서 `unreachable` 로 나오는데, 그것이 제대로 동작한 결과입니다 - 그 측정은 이
프로세스에서 Host 로 접속해 보는 것이고, 루프백에 열린 포트는 거기에 응답하지 않습니다.

주소를 바꾸면 그 Host 의 터널이 전부 다시 만들어집니다. 주소는 터널을 구성하는 값의
일부입니다.
...
정하지 않습니다. tunnel-manager 는 `<bind address>:<local port>` 를, Host 가 다른 주소를
