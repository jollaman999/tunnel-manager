...
- **是记录，不是一堆命令。** Host、服务端口和分配关系都是数据库里的行。协调循环让正在运行的
  隧道与保存的保持一致，断开的会重新建立；连续失败时每次把等待时间加倍，直到上限。一个状态
  页面显示期望运行的数量、已连接的数量和出错的数量。
- **开放范围按转发选择，并且实际测量。** 每条分配关系和每个本地转发都可以选择在所有接口上
  开端口，还是只在回环地址上开。对开在所有接口上的隧道，tunnel-manager 在隧道建立后自行连接
  转发端口，显示它是否应答，因为最终绑定到哪里由 SSH 服务器决定（OpenSSH 的 `GatewayPorts`）。
  SOCKS5 代理除非选了所有接口，否则开在回环地址上；代理和本地转发都可以用一个列表限制允许
  连接的客户端地址。
- **告警。** 隧道、本地转发或代理断开超过你设定的时间时，通过 Webhook 或邮件告警，恢复后再发
  一次。
- **设置集中在一处。** 设置存在数据库里，在浏览器里改。整套配置可以作为一个用密码加密的文件
  迁移。SSH 密码、私钥以及 Webhook 和邮件设置都用本安装的密钥文件加密保存。
...
| 章节 | 里面写了什么 |
|------|--------------|
| [工作原理](reference.zh.md#工作原理) | 调谐循环、分配关系、一条隧道的完整流程、本地转发，以及 Host 的 SOCKS5 代理 |
