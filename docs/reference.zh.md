...
也能访问。只有[本地转发](#本地转发)方向相反：端口开在本机上，连接从 Host 那边发出。
...
**`local_port` 在所有本地转发之间唯一**，不管属于哪台 Host，因为它们都在同一台本机上开端口。
用已被占用的端口创建本地转发会被 `409` 拒绝；和本服务器保存下来要监听的端口（`api_port`）相同的
也一样，启动时换到别的端口的话，和现在正在监听的端口相同的也一样。反过来也一样：通过设置页面保存或设置导入修改 `api_port` 时，如果新端口是某个本地转发
打开的端口，就用 `409` 拒绝，不保存任何内容。拒绝里带着那个本地转发和 `suggested_port`（比它大、
本地转发、已保存的 `api_port`、现在正在监听的端口和请求的 `api_port` 都没占用的第一个端口），设置页面会弹出一个窗口，
把本地转发移到别的端口，保存时也可以改为移动 API 端口。1024 以下的
...
| 设置 | 规则 |
|------|------|
| `api_port` | 1 到 65535。改成某个本地转发打开的端口会被 `409` 拒绝，见[本地转发](#本地转发) |
...
就可以再次打开了。如果有本地转发开着默认的 API 端口，下一次启动可能会在另一个端口上监听，
所以会用一条警告（`settings.default_port_held_by_forward`）指出这条转发。
