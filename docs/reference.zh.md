...
负责它的多台 Host 共用，本地转发不是。添加、修改、开启和关闭、删除在 Hosts 页面上那台 Host 所在行的
...
**关闭的本地转发仍然占着它的 `local_port`。** 它不开端口，也不建 SSH 连接，但上面的每一项检查都
像对待运行中的一样把它算进去：另一个本地转发、SOCKS5 代理或 `api_port` 要用这个端口，同样会被
`409` 拒绝，再次开启时也不会发现端口已经被别人占了。
...
| `status` | 含义 |
|----------|------|
| `disabled` | Host 已停用。不管本地转发开启还是关闭，什么都不打开 |
| `off` | Host 已启用，本地转发已关闭，什么都不打开 |
| `stopped` | Host 已启用，本地转发也已开启，但没有在运行的本地转发：调谐循环还没轮到这一行，或者启动失败了，失败的话日志里有 |
| `starting` | 正在建立第一次连接 |
...
`GET /api/host/:id/local-forward` 原来也用这样的数组返回这台 Host 的全部本地转发，现在收 `page` 和
`size`，并用这个形状作答，见[一台 Host 的本地转发](#一台-host-的本地转发)。
