...
`/api/status` 本来就是个对象。`tunnels` 现在是状态行的一页，两种转发同在一个列表里，`page`
和 `size` 排在它旁边，而**计数是对所有行算的，不是对这一页算的**：它们说的是本安装在做什么，
不是眼前这一页上有什么。

```bash
# 二十台一页的 Host 的第二页，以及状态行的最后一页：超过末尾会返回最后一页，
# 所以写一个很大的数就是在要最后一页。
...
`local` 是在 **Host** 上打开的地址，`remote` 是这边替它连上的服务；在本地转发的行上正好相反，
`local` 是在**本机**上打开的地址，`remote` 是从 Host 那边连到的目标。两者都是一个地址加一个
端口，看上去一模一样，上面的例子里是 `0.0.0.0:18080` 和 `127.0.0.1:15432`，除了 `kind` 没有
...
这两个也是对整套安装算的，不是对这一页算的。理由和前面那些计数一样，另外还多一条：一台负责四个
