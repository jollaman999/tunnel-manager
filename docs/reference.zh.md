...
| 页面 | 路径 | 显示什么、能做什么 |
|------|------|--------------------|
| Status | `/ui/status` | 三个计数（期望、行数、已连接）、一句话说明它们之间的差是怎么回事，以及每条隧道一行：Host、服务端口、状态、服务器、本地、远端、端口是否可达、重试次数、上次连上的时间。出了问题的隧道会在它下面横跨整张表再加一行写清哪里不对；转发端口无法连接的隧道，在那里写的是要在它指出的那台 SSH 服务器上修改什么、还要检查什么。隧道行是一页一页出的，一开始每页十行，页大小和页码在表格上方选；三个计数始终是所有隧道的计数，不是这一页的。它每 5 秒刷新一次，刷新后仍停在你正在看的那一页。 |
| Hosts | `/ui/hosts` | 每台 Host 一行，有 ID、IP、端口、用户、描述、是否启用和更新时间。行是一页一页出的，一开始每页十行，页大小（10、20、30、50 或 100）和页码在表格上方选。这个选择只记在这个页面上，而短到一页最小页大小就放得下的列表，索性连控件都不显示。可以添加 Host、编辑、启用或停用、删除。添加和编辑表单里有粘贴私钥的框、拖放密钥文件的区域，还有给带密码的密钥填密码的框；添加表单里有一个默认勾上的 **Assign all service ports**，它决定这台 Host 一开始负责什么。行里的 **Service ports** 会打开一个面板，列出所有服务端口，这台 Host 负责的那些已勾选；保存时只发送改动过的部分，所以在那里勾选一项，不会影响你没有查看的那些页。 |
...
进程结束才真正从磁盘上消失，那只能算做了一半。请手动删除它；如果这是作为服务安装
的，把 systemd unit 或者 compose 文件也一并删除。
...
**文件中以明文包含每台 Host 的 SSH 密码、私钥和密钥密码。** 这正是它的用途：数据库里那些内容是
用保存它们那台机器的加密密钥加密的，原样复制的文件在其他安装上一个也打不开。所以它们在导出
时被解密，再用接收方安装的密钥重新加密。这中间保护它们的，只有给整个文件设置的那个密码，
别无其他，所以请把导出的文件当作它里面每台 Host 的凭据来对待。
...
那台 Host 的隧道是重连而不是重建。文件里的每一行都会在响应里列成 `added`、`replaced` 或
`skipped`，决定要不要覆盖之前看的就是它。整个导入是一个事务：中途被拒绝的文件，留下的数据库
和原来完全一致。隧道本身不随文件走，因为调谐循环会根据 Host、服务端口和它们之间的分配关系建立
隧道。
...
| `forward_reach` | tunnel-manager 是否连接上了那个转发端口，方法是向 Host 的那个端口发起一条 TCP 连接：`reachable`、`unreachable`，还什么都没测过时是 `unknown` |
