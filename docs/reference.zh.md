...
**转发端口能到多远，是分配关系的一部分。**行上的 `bind_scope` 说的是请求把这个端口开在 Host 的
哪些地址上，它指的不是一个地址，而是**成对的一个范围**。

| `bind_scope` | 向 Host 请求的地址 | 用来做什么 |
|--------------|--------------------|------------|
| `loopback` | `127.0.0.1` 和 `::1` | 让这个端口只在 Host 本机上响应 |
| `wildcard`，以及空值 | `0.0.0.0` 和 `::` | 让这个端口在任何能够到达 Host 的地方都响应 |

**选定一个范围，两个地址都会请求。**因为两个地址族谁也代替不了谁：开在 `127.0.0.1` 上的端口，
用 `::1` 连过来的客户端是到不了的，两个通配地址也一样。选范围的人是为这台系统选的，不是为某一个
地址族选的，所以两个都请求，SSH 服务器接受了其中哪一个会写在隧道行上，见
[读懂隧道状态](#读懂隧道状态)。没有手工填地址的框：这个程序从不向 Host 询问它有哪些网卡，所以
写在这里的地址只是猜测，而猜错的结果就是一条永远打不开、也不会出声的转发。

范围由分配关系持有，是因为 Host 和服务端口两边都有发言权。一台 Host 负责好几个服务端口，其中
一个可能只该在这台系统内部使用，而另一个必须从外面够得着；一个服务端口由好几台 Host 负责，其中
只有一部分面向不该让外人进来的网络。放在任何一边，都会把一个答案强加给另一边。

这一列出现之前就存在的分配关系持有空值，而空值就是通配范围，所以升级不会收窄任何正在运行的隧道
的可达范围。在 v3.8.3 里给 **Host** 设过绑定地址的安装，那个答案会被带到这台 Host 的每一条分配
关系上：写的是回环地址就变成 `loopback`，其他答案，包括空值和本机某块网卡的地址，一律变成通配
范围。和分配关系本身只填一次一样，它只在添加这一列的那次启动时搬运，此后不再搬运，否则后面的
启动会覆盖掉这期间选定的结果。
...
sequenceDiagram
    ...
    rect rgb(255, 255, 220)
        ...
        Note right of Bastion: 每个已分配的服务端口，范围里的两个地址各一条：-R bindAddress:localPort:remoteIP:remotePort
    end
...
**在这一切之前，先要核对这台 Host 的主机密钥。** 核对是在握手里做的，所以只要不是这台 Host
信任的那台服务器，密码和私钥都不会递过去；没有人批准过密钥的 Host，一条隧道也建不起来。见
[主机密钥的批准](#主机密钥的批准)。

请求的地址是分配关系上 `bind_scope` 指的那两个，什么都没选就是通配地址，见
[分配关系](#分配关系)。两个都在同一条 SSH 连接上请求，所以**一条隧道持有两条转发**。服务器只开
其中一条、拒绝另一条时，这条隧道就只用所请求的一半在跑流量。行仍然只有一行，因为人选的就是一行。

Host 上的监听到底会不会开在所请求的地方，由 Host 上的 SSH 服务器说了算。它的 `GatewayPorts`
关闭时，不管请求的是什么，监听都绑定在回环地址上；打开时，即使选的是回环范围，也照样绑定到每块
网卡。隧道建立后，tunnel-manager 会自行测试那个转发端口，并报告结果，见
[转发端口是否可达](#转发端口是否可达)。
...
**本地转发属于一台 Host。** 它自成一行，只由创建它的那台 Host 负责，没有分配关系：服务端口由
负责它的多台 Host 共用，本地转发不是。添加、修改和删除在 Hosts 页面上那台 Host 所在行的
...
| `bind_scope` | 在本机的哪里打开 `local_port` |
|--------------|-------------------------------|
| `wildcard`，以及空值 | `0.0.0.0` 和 `::`。这是默认值 |
...
是用一个同时接收两个地址族的 `::` socket 打开的，所以别的程序不管在哪个地址上占着这个端口，都是 `error`。

**`local_port` 在所有本地转发和 SOCKS5 代理之间唯一**，不管属于哪台 Host，因为它们都在同一台
本机上开端口。用已被另一个本地转发或某台 Host 的 [SOCKS5 代理](#host-的-socks5-代理)占用的端口
创建本地转发会被 `409` 拒绝；和本服务器保存下来要监听的端口（`api_port`）相同的也一样，启动时换到
别的端口的话，和现在正在监听的端口相同的也一样。反过来也一样：通过设置页面保存或设置导入修改
`api_port` 时，如果新端口是某个本地转发或 SOCKS5 代理打开的端口，就用 `409` 拒绝，不保存任何内容。
拒绝里带着那个本地转发，或在 `socks_host` 里带着那台 Host，还有 `suggested_port`（比它大、本地
转发、SOCKS5 代理、已保存的 `api_port`、现在正在监听的端口和请求的 `api_port` 都没占用的第一个
端口），设置页面会弹出一个窗口，把本地转发或代理移到别的端口，保存时也可以改为移动 API 端口。1024 以下的
端口由这个进程自己打开，所以适用[以非 root 用户运行](#以非-root-用户运行)里的规则。

**本地转发只在它的 Host 启用时运行。** 调谐循环像对待隧道一样启动、重建和停止本地转发：一行就是
...
`connected` 表示 SSH 连接存在，`local_port` 已打开，不表示目标有应答：每个客户端连接都由 Host
单独去连目标，连不上时只关闭那一个连接，原因写进日志。Host 上的 SSH 服务器也必须允许这个方向的
转发；OpenSSH 在 `AllowTcpForwarding` 为 `no` 或 `remote` 时会拒绝。

| `status` | 含义 |
|----------|------|
| `disabled` | Host 已停用，什么都不打开 |
| `stopped` | Host 已启用，但没有在运行的本地转发：调谐循环还没轮到这一行，或者启动失败了，失败的话日志里有 |
| `starting` | 正在建立第一次连接 |
...
**状态保存在运行本地转发的那个进程的内存里**，只有本地转发的接口会返回它。`GET /api/status`
里没有，状态页也不统计本地转发。
...
**一台 Host 可以带一个 SOCKS5 代理。** 它做的就是 `ssh -D` 做的事，并像本地转发一样持续维护：
**本机**打开 `socks_port`，设成用它当代理的浏览器用 SOCKS5 的 `CONNECT` 请求某个网站，到那个网站
的连接就通过 Host 的 SSH 连接从 Host 那边发出。目标由浏览器在每个连接上自己指定，所以一个端口就
能到达 Host 能访问到的任何地方。主机名按收到的原样交给 Host，在 Host 那边解析，所以只在 Host
那边网络里认得的名字也能用。
...
flowchart LR
    ...
    target[("浏览器请求的网站<br/>主机名由 Host 解析")]

    tm ==>|"1. 通过 SSH 连接，然后打开 socks_port"| sshd
    browser -->|"2. 按主机名或地址发 SOCKS5 CONNECT"| port
    port -->|"3. 顺着 SSH 连接过去"| sshd
    sshd -->|"4. 解析主机名并连接"| target
```

**在 Hosts 页面添加和编辑 Host 的表单里开启。** 勾上 **启用 SOCKS5 代理** 后，会出现端口、开放
范围和 **允许的来源地址** 这几栏，端口预先填着 1080。通过 API 时，把同样四个字段放在 Host 上发送，
见 [Host](#host)。
...
`socks_allowed_sources` 收 IPv4 和 IPv6 地址以及 CIDR，用逗号或空格分隔，例如
`203.0.113.0/24, 198.51.100.7`。单个地址只代表它自己。写成 IPv6 形式的 IPv4 地址
`::ffff:192.0.2.1` 和 `192.0.2.1` 是同一个。读不懂的值，或者带 zone 的地址（如 `%eth0`），会被
`400` 拒绝，什么都不保存。

**来自列表之外地址的客户端会被立刻关掉**，一个字节都不读，也不回任何 SOCKS 应答。这类拒绝每个
代理每分钟最多记一次 `tunnel.socks_source_refused`，并带上距上一条以来拒绝的次数，所以端口被
扫描时日志也不会被淹没。

代理支持的是不带认证的 SOCKS5 `CONNECT`，目标可以是 IPv4 地址、IPv6 地址或主机名。

| 客户端请求的是 | 得到的是 |
|----------------|----------|
| `CONNECT` | 连接；Host 连不上时是 SOCKS 失败应答 |
| `BIND`、`UDP ASSOCIATE` | “不支持的命令”应答 |
| 用户名和密码，且不接受无认证方式 | 没有可接受方式的应答，随后关闭连接 |
| SOCKS4 或 SOCKS4a | 不回应答，直接关闭连接 |

Host 连不上的目标，会按 SSH 服务器给出的内容回一个 SOCKS 失败应答：连接被拒、网络或主机不可达、
SSH 服务器不允许，其他情况则是一般性失败。从 Host 发起的连接给 10 秒，客户端发来请求也给 10 秒。
这些失败是用浏览器时常有的事，所以只在 `debug` 级别记录。

**`socks_port` 在本机上唯一。** 开启代理或把它换到别的端口时，如果那个端口是另一台 Host 的代理
打开的、任何一台 Host 的本地转发打开的、本服务器保存下来要监听的（`api_port`），或者现在正在监听
的，就用 `409` 拒绝。开着的代理即使 Host 停用也占着那个端口，因为启用 Host 的那一刻它就会打开。
反过来也一样：本地转发和新的 `api_port` 都不能用代理占着的端口，见[本地转发](#本地转发)。1024
以下的端口适用[以非 root 用户运行](#以非-root-用户运行)里的规则。

**代理在它的 Host 启用**、且代理连同端口一起开启时运行。它用自己的一条 SSH 连接，和同一台 Host
的隧道、本地转发分开，由调谐循环像对待本地转发一样启动、重建和停止。代理的端口、范围、允许的
...
里用端口加状态标记显示它。没有代理的 Host 那一栏是 `-`，出错时错误显示在该行下面的一行。
...
**要用它，就让浏览器指向本机。** 以代理在 `192.0.2.20:1080` 为例：

- Firefox（以英文界面为准）：**Settings**、**Network Settings**、**Manual proxy configuration**。
  在 **SOCKS Host** 和 **Port** 填 `192.0.2.20` 和 `1080`，选 **SOCKS v5**，并勾上
  **Proxy DNS when using SOCKS v5**，这样主机名由 Host 解析，而不是由浏览器所在的机器解析。
- Chrome：在命令行上带着代理启动。
...
2. 初始密码写进一个叫 `initial-password` 的文件，**就放在数据库文件所在的目录里**，权限
   `0600`。在 Windows 上，它创建时不继承目录的 ACL，而是只让所有者、SYSTEM 和 Administrators
   能读取；以前的版本留下的这个文件会在启动时收窄到同样的 ACL。它是 52 个字符的大写字母和数字。
...
| 页面 | 路径 | 显示什么、能做什么 |
|------|------|--------------------|
| Status | `/ui/status` | 三个计数（期望、行数、已连接）、一句话说明它们之间的差是怎么回事，以及每条隧道一行：Host、服务端口、状态、服务器、本地、远端、端口是否可达、重试次数、上次连上的时间。出了问题的隧道会在它下面横跨整张表再加一行写清哪里不对；转发端口无法连接的隧道，多出来的那一行写的是要在它指出的那台 SSH 服务器上修改什么、还要检查什么。已建立的隧道，下面还会写出关于这条转发的地址已知的信息，把请求过的、SSH 服务器应答的、从这里发起的连接确认过的分开来写；它从不说端口是开着的。隧道行是一页一页出的，一开始每页十行，页大小和页码在表格上方选；三个计数始终是所有隧道的计数，不是这一页的。它每 5 秒刷新一次，刷新后仍停在你正在看的那一页。 |
| Hosts | `/ui/hosts` | 每台 Host 一行，有 ID、IP、端口、用户、描述、是否启用、SOCKS5 代理和更新时间。行是一页一页出的，一开始每页十行，页大小（10、20、30、50 或 100）和页码在表格上方选。这个选择只记在这个页面上，而短到一页最小页大小就放得下的列表，索性连控件都不显示。可以添加 Host、编辑、启用或停用、删除。添加和编辑表单里有粘贴私钥的框、拖放密钥文件的区域，还有给带密码的密钥填密码的框；添加表单里有一个默认勾上的 **Assign all service ports**，它决定这台 Host 一开始负责什么，旁边的 **在 Host 上的可达范围** 列表则是这个勾选所建立的全部分配关系的起点范围。行里的 **Service ports** 会打开一个面板，列出所有服务端口，这台 Host 负责的那些已勾选，每一行旁边还有它的可达范围：可以在上方选一个范围套用到所有勾选的行，也可以单独改一行，没勾的行不会被动；保存时只发送改动过的部分，所以在这个面板里勾选一项，不会影响你没有查看的那些页。行里的 **Local forwards** 会打开一个面板，列出这台 Host 的本地转发及各自的状态，在那里添加、修改和删除，见[本地转发](#本地转发)。添加和编辑表单还能开启这台 Host 的 SOCKS5 代理，那一栏显示端口和状态，见 [Host 的 SOCKS5 代理](#host-的-socks5-代理)。 |
...
这个端口不会是任何本地转发或 SOCKS5 代理要开的端口，并在日志 `api_server.port_taken_fallback` 里用
...
就可以再次打开了。如果有本地转发或 SOCKS5 代理开着默认的 API 端口，下一次启动可能会在另一个
端口上监听，所以会用一条警告（`settings.default_port_held_by_forward` 或
`settings.default_port_held_by_socks`）指出它。
...
| `GET` | `/api/host/:id/local-forward` | 这台 Host 的本地转发及其状态，见[一台 Host 的本地转发](#一台-host-的本地转发) |
...
| `bind_scope` | 可不填。取 `loopback` 或 `wildcard`，不填或发成空就是通配范围 | 不读。Host 本身不持有范围；单条分配关系的范围通过 `PUT /api/host/:id/service-port` 修改 |
...
**`bind_scope` 是这一次请求所建立的那批分配关系开在哪个范围上。**它只对这一批生效，别处不读。
Host 是在逐个决定服务端口之前注册的，所以这里给的这一个答案就是这一批的起点。请求把
`assign_all_service_ports` 发成 false 时不会建立分配关系，这个字段也就什么都不决定。它只收
`loopback` 和 `wildcard`，其他值以 `400` 拒绝，不填就是通配范围，这也是这套机制出现之前每条
转发所请求的值。

Host 本身不持有范围，所以 `PUT /api/host/:id` 不读这个字段，任何响应里也不会带它。单条分配关系
开在哪个范围上，在[一台 Host 负责的服务端口](#一台-host-负责的服务端口)里读取和修改；两个范围
各是什么意思，见[分配关系](#分配关系)。`socks_bind_scope` 是另一回事：它是 Host 的 SOCKS5 代理开在
本机的哪里。

把一条分配关系移到别的范围，只会重建这条分配关系的那一条隧道，因为范围是构成一条隧道的值的
一部分。同一台 Host 的其他分配关系继续照常运行。
...
| 字段 | 做什么 |
|------|--------|
| `add` | 要交给这台 Host 的服务端口。新建的分配关系按 `bind_scope` 写入 |
...
就会把别人钉在回环上的分配关系放宽成通配范围。一个根本没提到它的请求把可达范围放宽出去，是无论
...
| 既不是 `loopback` 也不是 `wildcard` 的 `bind_scope` | `400`，并指出是哪个字段。不写入任何内容 |
| 一个没保存过的 Host id | `404` |
...
认证，所以没通过检查的服务器，拿不到这台 Host 的 SSH 密码，也拿不到它的私钥。服务器出示的密钥
会被记下来，因为总要有个东西可以比对、可以批准；但它绝不会被记成受信任的密钥：哪把密钥才是对
的，这边看得见的东西说了不算，能说了算的只有亲自去服务器上读过指纹的人。

凡是带 Host 的响应都带 `host_key_fingerprint` 和 `pending_host_key_fingerprint`。前者是这台
Host 信任的密钥，后者是某台服务器在一次被拒绝的连接里出示的密钥。两者都是 SHA256 指纹，不是
密钥本身：指纹正是 `ssh` 首次连接时打印的东西，也是 `ssh-keygen -lf` 对服务器上那个主机密钥
文件报出来的东西，所以它是这把密钥唯一能拿去和机器本身核对的形式。还没有受信任密钥的 Host，
前者是空的；没有东西在等的 Host，后者是空的。

在等的 Host 处于两种状态之一，这两种不是同一个问题。是哪一种，写在那台 Host 的隧道行的
`status` 里，也写在下面这个列表的 `mismatch` 里。

| 状态 | 是什么 |
|------|--------|
| `host_key_unapproved` | 这台 Host 没有受信任的密钥。每台 Host 都从这里开始，在这项检查出现之前注册的 Host，升级之后也全都落在这里 |
| `host_key_mismatch` | 这台 Host 有受信任的密钥，而服务器出示了另一把。要么是服务器被重建过、换了新密钥，要么是这条连接根本没有到那台服务器，这两种在这边分不出来 |

| 方法 | 路径 | 做什么 |
|------|------|--------|
| `GET` | `/api/host-key` | 有密钥在等批准的 Host 的一页，覆盖整套安装。接受 `page` 和 `size`，见[分页](#分页) |
| `POST` | `/api/host/:id/host-key` | 批准一台 Host 上在等的密钥 |
| `POST` | `/api/host-key` | 一次请求批准多台 Host 的密钥 |

列表的每一行只带比对要用的东西，Host 的其他字段一概不带：一个面板一次放一百行，端口、用户和
说明在那里是一百行没人看的内容。
...
从未被批准过的 Host，`trusted_fingerprint` 是空的，`mismatch` 是 false。之所以单独带上
`mismatch`，是为了不让客户端拿那个字段去和空字符串比：批准时要不要问账号密码，就看它。

```bash
# 批准一台 Host 上在等的密钥。指纹要一起发回去，这样在列表读出来之后 SSH 服务器才出示的密钥，
# 就不会被一次冲着屏幕上那把密钥去的操作批准掉。
curl -s -b cookies.txt -X POST "$BASE/api/host/1/host-key" \
  -H 'Content-Type: application/json' \
  -H "X-CSRF-Token: $CSRF" \
  -d '{"fingerprint":"SHA256:<屏幕上的指纹>","password":"<账号密码>"}'
```

| 字段 | 接受什么 |
|------|----------|
| `fingerprint` | 必填。拿去和服务器核对过的那个指纹。只有在它仍然是这一行持有的密钥时，批准才会通过 |
| `password` | 是**这个账号**的密码，不是 Host 的密码。只有这台 Host 已经持有受信任的密钥时才需要，别处不读它 |

**还没有密钥的 Host，凭会话就能批准**，因为没有需要推翻的信任，而且这一下操作本来就在注册每台
Host 的路上。换掉一把受信任的密钥是另一回事：没有密码的话，一个开着没人看的会话，一下操作就会
让人去信任那个顶替服务器在应答的东西。

`POST /api/host-key` 就是把同一个批准施加到一个列表上，它的存在是因为升级之后的样子：在这项
检查出现之前注册的 Host，会在同一刻全都等着第一次批准。一台 Host 的指纹说明不了另一台，所以
请求里的每台 Host 各带自己的指纹；而密码是整个请求问一次，不是每台 Host 问一次。

```bash
curl -s -b cookies.txt -X POST "$BASE/api/host-key" \
  -H 'Content-Type: application/json' \
  -H "X-CSRF-Token: $CSRF" \
  -d '{"hosts":[{"host_id":2,"fingerprint":"SHA256:<屏幕上的指纹>"}]}'
```
  ...
  "data": {
    ...
    "hosts": [
      { "host_id": 2, "approved": true,
        "fingerprint": "SHA256:<这台 Host 从此信任的密钥>" }
    ]
  }
}
```

**一台被拒绝的 Host 不会把列表里其余的一起拖下水。** 它会回到 `hosts` 里，`approved` 是
false，并带上这次拒绝：`error`、`error_code`，句子里有值的还带 `error_args`，和一个说不的响应
带拒绝信息的形式一样；列表里其余的照常通过。一次请求最多列 **1000 台 Host**：请求里的每台 Host 都在同一个事务里读写，
而数据库只用一条连接跑，所以列表有多长，别的什么都得不到应答的时间就有多长。

| 发了什么 | 会怎么样 |
|----------|----------|
| 和在等的那个不一样的指纹 | `409`，并说明在等的是哪一个。什么都不写；在批量批准里，这只是那一台 Host 的拒绝 |
| 换受信任的密钥时没给密码，或者密码不对 | `401`。请求里的任何一台 Host 都不写 |
| 没有密钥在等的 Host | `409`。要么已经批准过了，要么自上次批准以来没有任何东西连过这台 Host |
| 没有保存过的 Host id | 单台批准是 `404`；批量批准里只是那一台 Host 的拒绝 |
| 一次请求里 Host 超过 1000 台 | `400`。什么都不读 |

批准会唤醒调谐循环，所以那台 Host 的隧道是当场重建，而不是等下一轮。只要还有东西在等，状态页
就会在表格上方挂一行，那一行是用 `GET /api/status` 里的两个计数画的，按下去打开的就是这个
列表。
...
| 方法 | 路径 | 作用 |
|------|------|------|
| `GET` | `/api/host/:id/local-forward` | 这台 Host 的全部本地转发及其状态，按 id 排序。不分页 |
| `POST` | `/api/host/:id/local-forward` | 给这台 Host 添加一个本地转发 |
| `GET` | `/api/local-forward/:id` | 读取一个本地转发 |
| `PUT` | `/api/local-forward/:id` | 修改一个本地转发。它所属的 Host 不变 |
...
这是之后 `GET /api/host/1/local-forward` 的返回。创建、读取和修改都用这样的一个对象作答。响应里
的 `bind_scope` 总是 `loopback` 或 `wildcard`，不会是空的。
...
| 计数 | 数的是什么 |
|------|------------|
| `host_keys_unapproved` | SSH 服务器出示了主机密钥，而自己还没有已批准密钥的 Host 有多少台 |
| `host_keys_mismatched` | 持有受信任密钥、却被出示了另一把的 Host 有多少台 |

这两个也是对整套安装算的，不是对这一页算的。理由和前面三个一样，另外还多一条：一台负责四个
服务端口的 Host 会因为同一把密钥被拒绝四次，按页来数，同一个问题就会出现四遍。在等的是这两个
数加起来的那个数，它们背后的列表是 `GET /api/host-key`，见[主机密钥的批准](#主机密钥的批准)。
...
`error_kind` 是给 `last_error` 属于哪一类失败起的名字，只给有地方可去的那一类：`forward_denied`
是 SSH 服务器拒绝打开转发端口。其余的失败，以及所有不是错误的行，这里都是空的。和
`forward_reach` 一样，它只说发生了什么，从不说为什么：让服务器拒绝的那几个设置，从这边看都
一模一样。

### 转发端口是否可达

写着 `connected` 的隧道，说明 SSH 连接已建立。这不等于转发端口可以连接：监听是由 **SSH
服务器**打开的，它绑定哪个地址是那台服务器的决定，不是这边的。tunnel-manager 请求的是分配关系
的绑定范围所指的那两个地址，没有选回环范围时，那就是通配地址 `0.0.0.0` 和 `::`。

**SSH 服务器对一次请求的应答，不是对它绑定了什么的测量。**转发请求的应答里只带端口，不带地址，
而这个程序也不会向那边问什么。决定绑定到哪里的，OpenSSH 是 `sshd_config` 里的 `GatewayPorts`，
Dropbear 是命令行上的 `-a` 标志。下面是对着 OpenSSH 实测的结果：

| `GatewayPorts` | 请求的地址 | 服务器实际绑定的 | 第二次请求 |
|----------------|------------|------------------|------------|
| `clientspecified` | `0.0.0.0` | `0.0.0.0` | `::` 成功打开 |
| `yes` | `0.0.0.0` | `0.0.0.0` 和 `::` | 拒绝 |
| `yes` | `127.0.0.1` | `0.0.0.0` 和 `::`，即所选的范围被忽略 | 拒绝 |
| `no` | `0.0.0.0` | `127.0.0.1` 和 `::1`，即所选的范围被忽略 | 拒绝 |

`man 5 sshd_config` 把 `yes` 写作 "force remote port forwardings to bind to the wildcard
address"，把 `no` 写作 "force ... available to the local host only"。**两者都是强制，都不问
客户端**，所选的范围能原样生效的设置只有 `clientspecified` 一个。拒绝也同样不等于端口是关着的：
在第一次请求里就把两个地址族都绑定了的服务器会拒绝第二次，完全没有 IPv6 的服务器也会拒绝，从
这里看是同一个应答。

每条隧道行上有四个字段，说明对这件事已知的信息。
...
**状态界面把三件事分开说，读的人也该这样分。**

| 说的是什么 | 依据是什么 |
|------------|------------|
| **请求过** | 分配关系的范围所指的那两个地址。是这边选的，所以确知 |
| **确认过** | 应答了从这里发起的 TCP 连接的地址，也就是 `forward_reach` 为 `reachable` 的情形。这是这边唯一握有的证据 |
| **不知道** | 其余全部，`open_reach` 也在内。答应不等于端口开着，拒绝也不等于端口关着 |

它从不说某个端口是开着的。对一个应答了从这里发起的连接的端口，能说的只是它在所拨的那个地址上
开着，仅此而已。
...
**`loopback` 范围的分配关系，从这里永远无法确认，这不是故障。**它的端口是请求开在 Host 自己的
地址上的，那台系统之外的任何东西都到不了，所以不管它在那边把流量跑得多好，从这里发起的连接都
不可能到达。把这种沉默记作 `unknown` 而不是 `unreachable`，正是因为这个：一条完全照要求在工作
...
密钥文件如果同组或其他人读得到，启动会拒绝继续。用 `chmod 600` 把权限收窄再启动。在 Windows 上，
创建和读取密钥文件时都会调整 ACL，使只有所有者、SYSTEM 和 Administrators 能读取；数据库文件、日志文件
以及本程序为它们创建的目录也一样。
...
- 1024 以下的 API 端口，非 root 进程无法绑定。请用 1024 或更大的端口，或者给可执行文件加上
  `CAP_NET_BIND_SERVICE`。本地转发的 `local_port` 也由这个进程自己打开，所以同样适用。
