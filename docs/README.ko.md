...
| 설정 | `/ui/settings` | 저장된 설정 전부와, 저장이 무엇을 바꿨고 그것이 지금 반영됐는지, 그리고 맨 아래에 제거. [설정](#설정) 참조 |
| 로그인 | `/ui/login` | 세션이 없는 클라이언트가 도착하는 화면. 첫 로그인에서는 사용자명을 비워 둡니다. 계정 설정이 아직이면 설정 화면으로 이어집니다. |

<!-- pending: 서비스 포트와 설정 사이에 들어갈 Log 화면. 만들어져서 직접 볼 수 있게 되면
     적습니다. -->
...
<!-- pending: Log 화면이 로그 파일을 읽어 오는 GET /api/logs. -->
